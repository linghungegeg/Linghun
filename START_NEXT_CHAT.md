...
当前任务状态：Phase 15 Pre-Smoke Full Reference Parity Audit + Permission / Architecture Runtime Feasibility + Source-of-Truth Reset 已完成，pre-smoke 证据基线仍为 `F:\Linghun\docs\audit\phase-15-pre-smoke-full-reference-parity-and-architecture-runtime-audit.md`；Batch A/B/C 已完成并通过 A-C 综合验收门，综合验收报告为 `F:\Linghun\docs\audit\phase-15-pre-smoke-a-c-combined-acceptance.md`。A-C 的 `READY_FOR_USER_DECISION_TO_START_REAL_PROJECT_SMOKE` 只作为历史证据，不再是当前直接 smoke 入口。用户已选择停止“边实测边补底座债”的节奏，当前唯一下一步是 `F:\Linghun\docs\delivery\pre-open-source-terminal-product-completion-gate.md`：按阶段完成开源前终端产品完整度门禁，然后再由用户决定是否进入真实环境全量实测。Phase 15.5 必须拆成 15.5A-F：A 性能与上下文（Workspace Reference Cache / Virtual Workspace Cache + Compact）、B 资源与任务生命周期、C 编辑与 MCP/codebase-memory、D Connect Lite、E provider/Freshness、F 终端产品 readiness；随后做 Phase 16 可控学习、Phase 17A local durable jobs、Phase 17B 企业微信/飞书/钉钉 remote channels 第一版。完成前不得启动真实全量实测、Phase 18 桌面端或开源发布。旧的 focused/mock/local/scoped PASS、Batch 3.5 PASS、单个 live text PASS、SKIPPED/PENDING smoke、silent-failure ban PASS、`READY_TO_IMPLEMENT`、Batch B 文档完成、A-C 综合验收 PASS 或单阶段 PASS 都不得推断终端产品 ready。
