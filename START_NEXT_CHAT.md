...
当前任务状态：Phase 15 Pre-Smoke Full Reference Parity Audit + Permission / Architecture Runtime Feasibility + Source-of-Truth Reset 已完成，pre-smoke 证据基线仍为 `F:\Linghun\docs\audit\phase-15-pre-smoke-full-reference-parity-and-architecture-runtime-audit.md`；Batch A/B/C 已完成并通过 A-C 综合验收门，综合验收报告为 `F:\Linghun\docs\audit\phase-15-pre-smoke-a-c-combined-acceptance.md`。A-C 的 `READY_FOR_USER_DECISION_TO_START_REAL_PROJECT_SMOKE` 只作为历史证据，不再是当前直接 smoke 入口。用户已选择停止“边实测边补底座债”的节奏，当前入口是 `F:\Linghun\docs\delivery\pre-open-source-terminal-product-completion-gate.md`：Phase 15.5A Performance & Context 已完成，报告为 `F:\Linghun\docs\delivery\phase-15-5a-performance-context.md`，完成范围仅代表 Workspace Reference Cache / Virtual Workspace Cache、Compact Lite boundary、cache/status/index fast path；不代表 Beta PASS，不代表 smoke-ready，未进入真实全量 smoke，未进入 Phase 15.5B-F / Phase 16 / 17 / 18。当前下一步可由用户决定是否进入 Phase 15.5B Resource & Task Lifecycle；后续仍需按阶段完成 15.5C-F、Phase 16、Phase 17A/17B 后，再由用户决定是否进入真实环境全量实测。完成前不得启动真实全量实测、Phase 18 桌面端或开源发布。旧的 focused/mock/local/scoped PASS、Batch 3.5 PASS、单个 live text PASS、SKIPPED/PENDING smoke、silent-failure ban PASS、`READY_TO_IMPLEMENT`、Batch B 文档完成、A-C 综合验收 PASS 或单阶段 PASS 都不得推断终端产品 ready。
...
- Phase 15 pre-Beta P0 hardening、Deep Parity、Runtime Acceptance、Verdict Evidence 和 CCB Maturity Remediation baseline 均作为证据输入；当前最新状态以“当前任务状态”段和最新 delivery/audit 报告为准。不得从历史 closure、focused/mock/local/scoped PASS 或单个 live text PASS 推断 Phase 15 Beta readiness PASS。当前真实全量实测已后移；必须先完成 Pre-Open-Source Terminal Product Completion Gate 中的 Phase 15.5A-F、Phase 16、Phase 17A/17B，并通过综合验收后，才由用户决定是否进入真实全量实测。
