...
pub struct ShellDeepLayerResult {
...
fn find_script(root: &Path) -> Option<PathBuf> {
    if let Ok(exe) = std::env::current_exe() {
        let candidate = exe.parent()?.join("shell-deep-layer.cjs");
        if candidate.exists() {
            return Some(candidate);
        }
    }
    let candidates = [
        PathBuf::from("shell-deep-layer.cjs"),
        root.join("shell-deep-layer.cjs"),
    ];
    for c in &candidates {
        if c.exists() {
            return Some(c.clone());
...
impl ShellDeepLayer {
    pub fn try_init(root: &Path) -> Result<Self, String> {
        let script = find_script(root).ok_or_else(|| "shell-deep-layer.cjs not found".to_string())?;
        ...
        Ok(ShellDeepLayer { child, stdin, stdout, root: root.to_path_buf() })
    }
    ...
    fn query(&mut self, root: &Path, files: &[String]) -> Result<Value, String> {
        ...
        let response = rx.recv_timeout(QUERY_TIMEOUT)
            .map_err(|_| "timeout waiting for shell-deep-layer response".to_string())?
            .map_err(|e| e)?;
...
pub fn run(layer: &mut Option<ShellDeepLayer>, root: &Path, files: &[String]) -> ShellDeepLayerResult {
    let start = Instant::now();

    if layer.is_none() {
        match ShellDeepLayer::try_init(root) {
            Ok(l) => *layer = Some(l),
            Err(reason) => {
                return ShellDeepLayerResult {
                    issues: vec![],
                    status: "unavailable",
                    reason: Some(reason),
...
mod tests {
    ...
    #[test]
    fn normalizes_only_explicit_shell_success_as_active() {
        assert_eq!(normalize_status("clean"), "active");
        assert_eq!(normalize_status("shell_error"), "active");
