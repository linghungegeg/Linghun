...
pub struct SwiftDeepLayerResult {
...
fn find_script(root: &Path) -> Option<PathBuf> {
    if let Ok(exe) = std::env::current_exe() {
        let candidate = exe.parent()?.join("swift-deep-layer.cjs");
        if candidate.exists() {
            return Some(candidate);
        }
    }
    let candidates = [
        PathBuf::from("swift-deep-layer.cjs"),
        root.join("swift-deep-layer.cjs"),
    ];
    for c in &candidates {
        if c.exists() {
            return Some(c.clone());
...
impl SwiftDeepLayer {
    pub fn try_init(root: &Path) -> Result<Self, String> {
        let script =
            find_script(root).ok_or_else(|| "swift-deep-layer.cjs not found".to_string())?;
    ...
    fn query(&mut self, root: &Path, files: &[String]) -> Result<Value, String> {
        ...
        let response = rx
            ...
            .map_err(|_| "timeout waiting for swift-deep-layer response".to_string())?
            .map_err(|e| e)?;
...
fn map_helper_status(status: &str, has_issues: bool) -> &'static str {
    match status {
        ...
        "swift_error" if has_issues => "active",
...
pub fn run(
    layer: &mut Option<SwiftDeepLayer>,
    root: &Path,
    files: &[String],
) -> SwiftDeepLayerResult {
    let start = Instant::now();

    if layer.is_none() {
        match SwiftDeepLayer::try_init(root) {
            Ok(l) => *layer = Some(l),
            Err(reason) => {
                return SwiftDeepLayerResult {
                    issues: vec![],
                    status: "unavailable",
                    reason: Some(reason),
...
mod tests {
    ...
    #[test]
    fn maps_only_confirmed_swift_results_to_active() {
        assert_eq!(map_helper_status("clean", false), "active");
        assert_eq!(map_helper_status("swift_error", true), "active");
