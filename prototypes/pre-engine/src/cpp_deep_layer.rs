...
fn find_script(root: &Path) -> Option<PathBuf> {
    if let Ok(exe) = std::env::current_exe() {
        let candidate = exe.parent()?.join("cpp-deep-layer.cjs");
        if candidate.exists() {
            return Some(candidate);
        }
    }
    let candidates = [
        PathBuf::from("cpp-deep-layer.cjs"),
        root.join("cpp-deep-layer.cjs"),
    ];
    for c in &candidates {
        if c.exists() {
            return Some(c.clone());
...
impl CppDeepLayer {
    pub fn try_init(root: &Path) -> Result<Self, String> {
        let script =
            find_script(root).ok_or_else(|| "cpp-deep-layer.cjs not found".to_string())?;
    ...
    fn query(&mut self, root: &Path, files: &[String]) -> Result<Value, String> {
        ...
        let response = rx
            ...
            .map_err(|_| "timeout waiting for cpp-deep-layer response".to_string())?
            .map_err(|e| e)?;
...
pub fn run(
    ...
    if layer.is_none() {
        match CppDeepLayer::try_init(root) {
            Ok(l) => *layer = Some(l),
            Err(reason) => {
                return CppDeepLayerResult {
                    issues: vec![],
                    status: "unavailable",
                    reason: Some(reason),
