...
pub struct CsharpDeepLayerResult {
...
fn find_script(root: &Path) -> Option<PathBuf> {
    if let Ok(exe) = std::env::current_exe() {
        let candidate = exe.parent()?.join("csharp-deep-layer.cjs");
        if candidate.exists() {
            return Some(candidate);
        }
    }
    let candidates = [
        PathBuf::from("csharp-deep-layer.cjs"),
        root.join("csharp-deep-layer.cjs"),
    ];
    for c in &candidates {
        if c.exists() {
            return Some(c.clone());
...
impl CsharpDeepLayer {
    pub fn try_init(root: &Path) -> Result<Self, String> {
        let script = find_script(root)
            .ok_or_else(|| "csharp-deep-layer.cjs not found".to_string())?;
        ...
        Ok(CsharpDeepLayer { child, stdin, stdout, root: root.to_path_buf() })
    }
    ...
    fn query(&mut self, root: &Path, files: &[String]) -> Result<Value, String> {
        ...
        let response = rx.recv_timeout(QUERY_TIMEOUT)
            .map_err(|_| "timeout waiting for csharp-deep-layer response".to_string())?
            .map_err(|e| e)?;
...
pub fn run(layer: &mut Option<CsharpDeepLayer>, root: &Path, files: &[String]) -> CsharpDeepLayerResult {
    let start = Instant::now();

    if layer.is_none() {
        match CsharpDeepLayer::try_init(root) {
            Ok(l) => *layer = Some(l),
            Err(reason) => {
                return CsharpDeepLayerResult {
                    issues: vec![],
                    status: "unavailable",
                    reason: Some(reason),
...
mod tests {
    ...
    #[test]
    fn normalizes_only_explicit_csharp_success_as_active() {
        assert_eq!(normalize_status("clean"), "active");
        assert_eq!(normalize_status("csharp_error"), "active");
