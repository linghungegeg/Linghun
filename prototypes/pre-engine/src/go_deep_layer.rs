...
impl GoDeepLayer {
    pub fn try_init(root: &Path) -> Result<Self, String> {
        ...
        Ok(GoDeepLayer { child, stdin, stdout, root: root.to_path_buf() })
    }
    ...
    pub fn query(&mut self, files: &[String]) -> Result<(Vec<Value>, u128), String> {
        ...
        let resp_line = match rx.recv_timeout(QUERY_TIMEOUT) {
            ...
            Err(_) => {
                self.child.kill().ok();
                return Err("timeout: go helper did not respond within 30s".to_string());
            }
        };
...
pub fn run(deep: &mut Option<GoDeepLayer>, root: &Path, files: &[String]) -> GoDeepLayerResult {
    if deep.is_none() {
        match GoDeepLayer::try_init(root) {
            Ok(d) => *deep = Some(d),
            Err(reason) => {
                return GoDeepLayerResult {
                    issues: vec![],
                    status: "unavailable",
                    reason: Some(reason),
                    elapsed_ms: 0,
    ...
    match d.query(files) {
        Ok((issues, elapsed_ms)) => GoDeepLayerResult {
            issues,
            status: "active",
            reason: None,
            elapsed_ms,
