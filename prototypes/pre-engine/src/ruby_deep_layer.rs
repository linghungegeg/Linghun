...
pub struct RubyDeepLayerResult {
...
fn find_script(root: &Path) -> Option<PathBuf> {
    if let Ok(exe) = std::env::current_exe() {
        let candidate = exe.parent()?.join("ruby-deep-layer.cjs");
        if candidate.exists() {
            return Some(candidate);
        }
    }
    let candidates = [
        PathBuf::from("ruby-deep-layer.cjs"),
        root.join("ruby-deep-layer.cjs"),
    ];
    for c in &candidates {
        if c.exists() {
            return Some(c.clone());
...
impl RubyDeepLayer {
    pub fn try_init(root: &Path) -> Result<Self, String> {
        let script = find_script(root)
            .ok_or_else(|| "ruby-deep-layer.cjs not found".to_string())?;
        ...
        Ok(RubyDeepLayer { child, stdin, stdout, root: root.to_path_buf() })
    }
    ...
    fn query(&mut self, root: &Path, files: &[String]) -> Result<Value, String> {
        ...
        let response = rx.recv_timeout(QUERY_TIMEOUT)
            .map_err(|_| "timeout waiting for ruby-deep-layer response".to_string())?
            .map_err(|e| e)?;
...
fn map_helper_status(status: &str, has_issues: bool) -> &'static str {
    match status {
        ...
        "ruby_error" if has_issues => "active",
        "tool_missing" | "unavailable" => "tool_missing",
        _ => "partially_verified",
    }
}

pub fn run(layer: &mut Option<RubyDeepLayer>, root: &Path, files: &[String]) -> RubyDeepLayerResult {
    let start = Instant::now();

    if layer.is_none() {
        match RubyDeepLayer::try_init(root) {
            Ok(l) => *layer = Some(l),
            Err(reason) => {
                return RubyDeepLayerResult {
                    issues: vec![],
                    status: "unavailable",
                    reason: Some(reason),
...
mod tests {
    ...
    #[test]
    fn maps_only_confirmed_ruby_results_to_active() {
        assert_eq!(map_helper_status("clean", false), "active");
        assert_eq!(map_helper_status("ruby_error", true), "active");
