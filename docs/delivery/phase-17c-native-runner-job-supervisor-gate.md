...
- 新增 Runner Adapter 最小 start/status/stop 路径：available 时执行受控 placeholder approved task；unavailable/mismatch/start failure 显式进入 Node/TUI fallback。
- durable job 报告、status、primary output、background/details 输出都写入 runner summary；runner lifecycle completed 仍是 partial，不是 verification PASS。
- cancel/recovery/status refresh 写回 runner terminal state；cancelled/timeout/stale/failed/protocol mismatch/start failure 不生成 PASS evidence。
...
- DONE：durable job / background / report / log artifact / resource guard 可见 runner 状态。
- DONE：runner completed 不等于 verification PASS；failed/timeout/cancelled/stale/protocol mismatch/start failure 不生成 PASS evidence。
- DONE：Windows 中文/空格路径作为 focused test 边界覆盖；mock runner 路径包含空格和中文目录。
- DEFERRED：managed package / bundled binary distribution、真实 daemon/long-running supervision、official release signing/AV/install matrix、Unix/macOS process-group cleanup、真实 Linghun workload benchmark、长期 native heartbeat owner-death benchmark。
...
- `start`：available 时调用 runner `start --id --root --timeout-ms -- node -e process.exit(0)`，这是 Phase 17C 第一版 gated lifecycle/protocol path，不声称真实 long-running daemon supervision。
- `status`：`/job status` / `/job report` 时 refresh native status。
- `stop`：`/job cancel` 时调用 runner stop，并将 runner/job 标记 cancelled，non-PASS。
- fallback：disabled/unavailable/protocol_mismatch/start_failed 都显式记录 `adapter=node`、`status=node_fallback`、`fallbackReason`。
...
- `/job run` 在 job 进入 running 后启动 runner adapter，再继续 Phase 17A bounded worker loop。
- `/job status`、`/job report` refresh runner status 并写回 state/report/background。
...
Adapter 不转发用户输入中的 raw command。Focused test 用 goal 中的 `rm -rf secret` 验证 runner start argv 只包含 approved placeholder task，不包含用户 raw command 或 secret 文本。

### Minimal lifecycle adapter, not full daemon

本轮 adapter 使用 native prototype 的 `start/status/stop` lifecycle protocol，但 `start` 当前只运行受控 placeholder `node -e process.exit(0)`，随后 durable job 仍由 Phase 17A bounded worker loop 执行。真实 long-running daemon supervision 后置。
...
- 当前 adapter start 只运行受控 placeholder approved task，不是真实 long-running durable job daemon supervision。
...
Phase 17C 当前本地实现和 focused validation 已闭合。用户已明确要求停止 independent verifier，改由本轮本地复检闭合；用户可决定是否进入 final full audit / polish / smoke pre-acceptance。不得自动进入，也不得自动执行真实全量 smoke 或宣布 Beta/open-source readiness。

下一阶段若启动，应只做最终全量审计/打磨/smoke pre-acceptance：整理阶段报告一致性、跑用户明确批准的完整验证矩阵、修复 blocking polish 问题。不得把 Fast Workspace Scanner 或 Phase 18 desktop 混入。
...
- nextPhase: user decision before final full audit / polish / smoke pre-acceptance.
- prohibited:
...
- evidence:
  - focused config/TUI vitest commands above
  - full config/TUI test commands above
...
当前未发现 Phase 17C blocking 问题。仍需 independent verifier 复检通过后，才能向用户报告 Phase 17C 完成状态。若 verifier 通过，用户可决定是否进入 final full audit / polish / smoke pre-acceptance。
