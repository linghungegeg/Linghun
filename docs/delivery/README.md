...
| Phase 15 preflight | done; Beta readiness PARTIAL/BLOCKED | [phase-15-natural-command-bridge.md](./phase-15-natural-command-bridge.md)；当前唯一 pre-smoke 基线：[phase-15-pre-smoke-full-reference-parity-and-architecture-runtime-audit.md](../audit/phase-15-pre-smoke-full-reference-parity-and-architecture-runtime-audit.md)；A-C 综合验收：[phase-15-pre-smoke-a-c-combined-acceptance.md](../audit/phase-15-pre-smoke-a-c-combined-acceptance.md) | 自然语言控制桥 + preflight hardening + pre-Beta cleanup + Interaction Maturity Fix + Full Interaction P0 hardening 已完成；历史 P0-1~P0-6、CCB handfeel gate、runtime acceptance、Verdict Evidence Gate、Batch 1/2/3 与 Batch 3.5 等只作为 evidence。当前唯一 pre-smoke 执行基线仍是 2026-05-21 `phase-15-pre-smoke-full-reference-parity-and-architecture-runtime-audit.md`；Batch A/B/C 已完成并通过 A-C 综合验收门，当前 verdict 为 `READY_FOR_USER_DECISION_TO_START_REAL_PROJECT_SMOKE`。这不是 Beta PASS，也不是 smoke-ready PASS；未进入真实项目 smoke，未进入 Phase 15.5 / Phase 16+。下一步只能在用户明确确认后进入 Phase 15 real-project smoke；用户确认前不得启动真实项目 smoke。不得从 focused/mock/local/scoped PASS、Batch 3.5 PASS、单个 live text PASS、SKIPPED/PENDING smoke、silent-failure ban PASS、`READY_TO_IMPLEMENT`、Batch B 文档完成或 A-C 综合验收 PASS 推断 Beta readiness PASS。历史 audit 只作为 evidence，不作为当前执行入口；历史 PASS / READY / READY_TO_FIX / READY_FOR_USER_DECISION 必须被本 README、START_NEXT_CHAT、delivery README、蓝图和当前 pre-smoke / A-C 综合验收报告覆盖 |
