...
Phase 15.5 / 16 / 17 每个实现阶段还必须先做 Source-Level Reality Check：

- 读取本阶段 source-of-truth、`docs/audit/reference-map.md`、最新 pre-smoke / A-C / gate 文档和相关历史审计。
- 优先用 codebase-memory 索引定位现有实现；Linghun 仓库索引项目名为 `F-Linghun`。索引不可用或过期时，降级为 `rg` 和精读关键源码。
- 输出 existing implementation / gaps / minimal touch points / forbidden duplicate systems，作为实现前置证据。
- 不能只凭文档新造 runtime；已有实现必须优先复用和补齐。
- 对照参考源和 baseline 第 12/13 节，把本阶段相关小细节复制进阶段 scope 并裁决 DONE / DEFERRED / NOT-DO；terminal-scope 成熟度遗漏应尽量当轮补齐，避免最后审计集中还债。
- 若源码事实与文档设计冲突，不直接扩大实现范围；先按源码事实给最小修正文档口径或请求用户确认。
...
- 默认最多 3 个真实运行 agent 作为保守基线；Phase 17A 可通过资源预算、任务类型 cap 和 benchmark 放宽到更高并发。8 agent 只作为高配/压测候选目标，不是默认承诺，也不是无限并发。
- agent 输出摘要化回主线程。
...
用户可见输出必须作为产品手感验收项，而不是文案美化项：主屏和普通最终回复必须 summary-first、human-first、action-first，默认只说事实结果、影响范围、用户选择、验证状态、下一步和详情路径。内部审计/架构/证据/权限字段默认回归底层，`systemic_gap`、`blocking_P1`、`DOC-ONLY`、`Architecture Card`、`Source-Level Reality Check`、raw evidence、raw tool_result、raw flags、gateId 等不得出现在普通主屏；这些内容只能进入 details/debug/report。模型和本地提示不得说 AI 话或空泛工程话，必须用具体命令、路径、状态、限制和可执行建议表达。
...
Phase 15.5 现在属于真实全量实测前的 Pre-Open-Source Terminal Product Completion Gate，不再等待 Phase 15 真实项目测试完成后才启动。本阶段按 15.5A-F 拆分推进，把性能与上下文、资源与任务生命周期、编辑与工具体验、Connect Lite、provider/Freshness、终端产品 readiness 逐项收口，降低把真实实测变成底座技术债探测器的风险。本阶段不新增 Phase 16+ 产品功能，不提前进入 Phase 16。
...
2026-05-21 追加：A-C 综合验收的 `READY_FOR_USER_DECISION_TO_START_REAL_PROJECT_SMOKE` 只作为历史 evidence。用户已选择把真实环境实测后移到终端候选产品成熟之后。当前执行入口升级为 Pre-Open-Source Terminal Product Completion Gate：按阶段完成 Phase 15.5 终端 hardening / Connect Lite / provider / Freshness / release runtime readiness、Phase 16 可控学习、Phase 17A local durable jobs、Phase 17B 企业微信/飞书/钉钉 remote channels 第一版、Phase 17C Native Runner / Job Supervisor Gate。该门禁完成并综合验收前不得启动真实全量实测、Phase 18 桌面端或开源发布。Phase 18 桌面端完整实现、开源发布物料、插件/skill 市场、云同步、自动更新、商业化账号、个人微信和完整远程工作台继续后置。

Phase 15.5 也必须复检 Solution Completeness Gate：双模型审查、pre-smoke 报告、A-C 综合验收和后续真实实测报告中出现的缺陷，必须先分类为单点修复、系统性缺口、后续登记或不做。系统性缺口要给最小完整修复边界，避免把 Phase 15.5 变成无限补体验细节。
...
Phase 15.5 必须登记并收口 **Local Resource Guard Lite**，但不得把它回流成当前 A-C 后真实项目 smoke 的默认 blocker，除非真实 smoke 已经证明系统卡顿、后台任务堆积、子进程残留或长输出占内存直接污染普通请求链路。该项的参考取舍是：以 CCB / Claude Code 的 task lifecycle 手感为主，吸收 TaskOutput / TaskStop、输出落盘、后台状态、可取消和 terminal status 先于慢 cleanup 的行为边界；以 OpenAI Codex / Codex CLI 的 sandbox、approval profile、subagent `max_threads` 和“并行会增加 token / latency / local resource pressure”的公开边界作为并发克制约束。Linghun 的优化方向是新手无感：默认前台模型请求并发为 1，后台任务有全局上限和 bash / verify / index / agent 分类型上限，index refresh、full test/build、verify、agent batch 等重任务默认不互相并跑，输出默认落盘且主屏 summary-first，`/background` 能显示 running / stale / timeout / cancelled / log path，Windows 进程树清理单独验收，cancelled / timeout / stale 不得写成 PASS。
...
2026-05-21 pre-smoke 状态口径：`docs/audit/phase-15-pre-smoke-full-reference-parity-and-architecture-runtime-audit.md` 仍是 pre-smoke 证据基线；Batch A/B/C 已完成并通过 A-C 综合验收门，综合验收报告为 `docs/audit/phase-15-pre-smoke-a-c-combined-acceptance.md`。A-C 的 `READY_FOR_USER_DECISION_TO_START_REAL_PROJECT_SMOKE` 只作为历史 evidence，不再是当前直接 smoke 入口。当前唯一下一步是 `docs/delivery/pre-open-source-terminal-product-completion-gate.md`：完成终端产品完整度门禁并通过综合验收后，再由用户决定是否进入真实环境全量实测。这不是 Beta PASS，也不是 smoke-ready PASS；门禁完成前不得启动真实全量实测、Phase 18 桌面端或开源发布。不得从 focused/mock/local/scoped PASS、Batch 3.5 PASS、单个 live text PASS、SKIPPED/PENDING smoke、silent-failure ban PASS、`READY_TO_IMPLEMENT`、Batch B 文档完成、A-C 综合验收 PASS 或单阶段 PASS 推断终端产品 ready。后续阶段接手时除 baseline 第 12/13 节外，还必须复制最新 pre-smoke 报告、A-C 综合验收报告、终端产品完整度门禁文档和 reconciliation 中的 pull-forward / keep-deferred 决策。历史 audit、历史 PASS、READY、READY_TO_FIX、READY_FOR_USER_DECISION 和聊天记忆只能作为 evidence，必须被 README、START_NEXT_CHAT、docs/delivery/README、本蓝图、最新 pre-smoke / A-C 综合验收报告和终端产品完整度门禁文档覆盖。
...
- Phase 15.5 Local Resource Guard Lite：收口本机资源保护的轻量底座，覆盖前台模型请求并发 1、后台任务全局 cap、bash / verify / index / agent 分类型 cap、重任务互斥、输出落盘不常驻内存、timeout / cancel / stale 状态、Windows 进程树清理、`/background` 非阻塞查看和 cancelled / timeout / stale 不得 PASS。若真实 smoke 证明资源压力污染默认链路，则升级为 Phase 15 遗漏/P1 回补；否则作为开源前 hardening。
...
Phase 15.5C 完成后可以追加两个轻量底座增强小阶段，用于开源前降本增效，但不得膨胀为新平台：

- **Log Artifact Runtime Lite**：把 Bash、verification、background output 从“只有 logPath/fullOutputPath”升级为可 bounded tail / grep / errors 的只读 artifact。它为反幻觉、review、Problems panel、Context Picker 和后续 durable jobs 提供真实日志切片和错误候选；实现必须用 TS/Node Lite，限制 known log/output roots、输出脱敏、maxBytes/maxLines/maxMatches/timeout 有界，errors 只标记 extracted candidates，不做 root cause，不改变 PASS evidence 规则，不做 native/binary、sqlite/数据库、后台索引或模型自动总结日志。
- **Workspace Snapshot Lite**：在现有 Workspace Reference Cache 基础上增加更可复用的 bounded 文件状态摘要，供 Project Doctor、Context Picker、Architecture Runtime 和 Phase 17A agent/job 共享，减少重复扫描和旧上下文误判。它只记录 stat/hash/ignore/top-level directory/changed summary 等轻量 metadata 和 refs，不缓存完整源码、完整日志、完整索引或私密信息；不得替代 codebase-memory，不做自研代码图索引、LSP、全仓 watcher、后台常驻扫描或 native/binary。
...
- Local Resource Guard Lite：多次模型请求不会前台并发执行；超过后台任务全局或分类型上限时给人话提示和可选下一步；index refresh、verify、full test/build 和 agent batch 等重任务默认互斥或要求确认；任务超时、取消、stale、Windows 进程树清理和长输出落盘都有 focused tests；`/background` 能非阻塞显示运行中、卡住、超时、取消和日志路径。
...
Phase 15.5F 还必须补齐几项实测前高收益 Lite 能力，全部复用现有 runtime，不新增重平台：

- **Project Doctor Lite**：在首次进入项目、`/doctor`、`/status`、`/index status` 或 Architecture Runtime facts 不足时，生成短 Project Facts：技术栈、包管理器、启动/测试/构建命令、关键配置、CI/脚本入口、风险目录和已知 unknown。不得做 onboarding wizard，不得全仓常驻扫描；信息来源必须来自 README/package/config/CI/索引/evidence，缺失写 unknown。
- **Context Picker Lite**：把用户的“看刚才报错 / 看这次 diff / 看这个模块 / 看项目怎么跑”等自然语言映射到已有上下文来源：changedFiles、diff、lastVerification、background log、evidence refs、Workspace Reference Cache、codebase-memory refs、Project Facts。只传短摘要和 refs，不把完整源码、完整日志或完整索引塞进 prompt。
- **Verification Command Discovery Lite**：复检 Phase 08 / 15.5B 已有验证命令探测能力，确保至少覆盖当前 JS/TS repo 的 `package.json` scripts、`tsconfig`、Biome/Vitest/pnpm/corepack、项目规则文件和 CI 线索；无法确定时输出候选和 unknown，不盲跑重命令。
- **Source-of-Truth Drift Linter Lite**：在最终 gate 前扫描 active docs，检查旧 `READY/PASS` 是否污染当前 readiness、`DOC-ONLY` 是否冒充 runtime DONE、Phase 00-14 是否被后续成熟度回写、source-of-truth 是否互相冲突。只做静态文档/报告口径检查，不新增大型审计平台。
- **Rollback Coach**：复用已有 checkpoint / rewind，在跨文件或高风险编辑前后给短提示：checkpoint id、changedFiles、恢复命令、是否建议先 commit。不得自动 commit，不得扩大 Git 工作流。
- **Task Cost Preview Lite**：Start Gate / Architecture Card / 长任务轻提示中给出轻量资源预估：轻/中/重、可能读取的上下文来源、是否会开 agent、是否会跑验证、是否联网、是否重任务互斥。它不是账单预测，不额外调用模型，不显示虚假金额。
- **Problems panel Lite**：归入现代终端 panel/polish，不接完整 LSP、不常驻诊断、不做 problem DB；只从 `/verify`、typecheck、lint、test、build、doctor 输出中提取问题摘要和 refs，通过 `/problems`、`/verify last` 或 `/details verification` 查看。

明确不做：

- Sandbox profile / 容器化执行环境平台。服务器和本机环境天然不同，第一版只约束本地权限、路径、同步提示和 doctor，不用 sandbox profile 假装环境等价。
- 直接在线上/服务器工作区改文件。第一版必须把“本地改动、本地验证、再同步到服务器”作为底层交付纪律：Linghun 可以给同步命令、部署检查和回滚建议，但不得默认 SSH 到服务器直接编辑文件；服务器改动必须由用户显式确认、说明影响范围，并优先要求先在本地仓库形成可追踪 diff。
- Recipe / Runbook 平台。Architecture Runtime、Natural Command Bridge、help/workflow 已覆盖主要新手引导；不得再新增复杂 recipe 市场、runbook runtime 或模板系统。
...
- Local Resource Guard Lite 有独立小节：列出 CCB / Claude Code task lifecycle 与 OpenAI Codex / Codex CLI 并发/权限边界的取舍，验证前台并发、后台任务 cap、分类型 cap、重任务互斥、输出落盘、timeout/cancel/stale、Windows process cleanup、`/background` 非阻塞和非 PASS 口径；若只有限时 kill 或展示上限，必须保守标为 PARTIAL。
...
- CCB Agent 生命周期。
- CCB Cron / job 类能力。
- CCB 的并发边界：前台 query guard、安全工具并发 / 非安全工具串行、background task 输出落盘、owner/stale recovery、bounded queue / Map / Set。
- OpenAI Codex / Codex CLI 公开 subagent 边界：`max_threads` / `max_depth` 这类并发上限、并行增加 token / latency / local resource pressure 的约束。
- Claude Code / CCB 公开 agent / subagent / worktree 方向：多 agent 可提升吞吐，但必须隔离写入、控制预算、避免长输出和重复上下文污染主会话。
...
- 自动会话 handoff packet。
- 自动 agent 分工。
- Virtual Agent Concurrency：多 agent / 多 job 的低资源调度层。
- Remote Channels。
...
- **Phase 17A local durable jobs + Virtual Agent Concurrency**：只要求本地长期任务、定时唤醒、后台会话、handoff 校验、预算/超时/取消、风险暂停、job report、本地 TUI 状态可见，以及低资源多 agent 调度成熟度。17A 通过后才算具备可控长期托管底座。
- **Phase 17B remote channels / adapters**：只在 17A 稳定后开启，默认关闭；第一版只做企业微信 / 飞书 / 钉钉 official_cli 或官方 webhook adapter。先做一个最小通知或审批 adapter / mock adapter，证明脱敏、去重、过期、来源校验和失败降级，再考虑更多能力；其他通道标记 future / unsupported。
- **Phase 17C Native Runner / Job Supervisor Gate**：只在 17A/17B 之后作为独立门禁接入长任务执行与监督底座。17C 不是更会写代码的 agent，也不是替换 Node 默认路径；Node/TUI 继续负责短任务和默认路径，Native Runner 只承接长任务、复杂进程树、durable job、多 agent 监督和可恢复性。17C 必须实现 Runner Resolver、Runner Adapter、Node fallback、`/doctor runner`、Windows 进程树清理、heartbeat/log/status supervisor、crash/missing/protocol mismatch fallback、scheduler/evidence/resource guard/log artifact/handoff recovery 集成，以及中文/空格路径和跨平台 process cleanup 验证。
...
- Phase 17A 的 Virtual Agent Concurrency 不得新造第二套 provider/tool/permission/evidence runtime；它只能在既有 Agent、BackgroundTask、Workspace Reference Cache、codebase-memory、evidence、handoff 和 Local Resource Guard 上做调度。
- Phase 15.5B 只为 Virtual Agent Concurrency 打地基：前台模型请求并发 1、后台任务 cap、分类型 cap、重任务互斥、timeout/cancel/stale、输出落盘和非 PASS 口径。不得把 17A 的多 agent 调度、durable job 图、remote channel 或长期自治提前塞进 15.5B。
- Phase 17A 的并发策略必须是智能调度，不是固定锁死 3 个，也不是默认无限开。3 个真实运行 agent 是保守默认；更多 agent 可以处于 queued / sleeping / blocked；8 agent 只作为高配/压测候选目标，必须以同场景 benchmark 证明前台不卡顿、资源占用可控、日志/取消/timeout/Windows 进程树清理稳定后再开放。
- Native Local Job Runner / Process Supervisor 在 17C 之前只能保留为 prototype/evaluation input，不能接入主链路或宣称 17A/17B ready。17C 接入时 runner 只能执行 Linghun 已批准的 job spec，不能绕过 Start Gate / permission pipeline，不能自己成为第二套 scheduler / agent runtime；completed / failed / timeout / cancelled / stale / crash 必须通过 Evidence Bridge 映射，非 PASS 状态绝不生成 PASS evidence。
- Fast Workspace Scanner 当前不得作为 Phase 17A 必做项或 codebase-memory 替代项。它只能作为 post-runner、benchmark-gated、optional managed native helper 候选：默认路径仍是 TS/Node Workspace Snapshot Lite 提供 bounded filesystem metadata，codebase-memory 提供 code graph/index intelligence。只有当大仓库、多 agent/job 共享 metadata、Project Doctor / Context Picker / Architecture Runtime 等场景证明 TS/Node snapshot 成为瓶颈，并且 managed binary / doctor / fallback / packaging 路线已被 Runner 证明可控时，才允许进入 scanner prototype。
...
- 是否允许 Bash。
- 是否允许多 agent。
- 多 agent 只表示用户意图和任务分解，不等于真实无限并发。runtime 必须按资源预算决定 runnable / running / sleeping / blocked / completed / cancelled / stale。
- 是否需要先输出 plan。
- 是否需要用户审批后写入。
...
- agent 上下文必须懒加载：默认传短任务摘要、项目 facts、索引/evidence/log 引用和必要文件摘要；不得给每个 agent 注入完整历史聊天、完整源码、完整 tool output 或完整索引结果。
- 多 agent 必须共享 Workspace Reference Cache、codebase-memory status、evidence refs、tool result summary 和已知 project facts；禁止每个 agent 独立重复全仓扫描或重复构建大上下文。
- 调度器必须有全局模型请求上限、前台模型请求上限、工具任务上限和 bash / verify / index / agent 分类型上限；full test/build、index refresh、verification、agent batch 等重任务默认互斥或要求用户确认。
- 调度器必须区分“用户创建的 agent / job 数量”和“真实运行并发数”：用户可以创建超过 3 个 agent / job，但 runtime 可以让部分保持 queued / sleeping / blocked。默认真实运行数从 3 起步；8 agent 是压力测试和高配机器候选配置，不能作为开源默认宣传承诺。
- agent / job 结果只以结构化摘要、evidence refs、验证结果和采纳/拒绝状态回到主消息流；原始长输出只能进 logPath / fullOutputPath / transcript。
- cancelled / timeout / stale / blocked 的 agent 或 job 不能贡献 PASS evidence；需要恢复时必须先说明 stale reason、owner/session、最后 heartbeat、日志路径和可选 next action。
...
- Remote Channels 第一版只支持企业微信 / 飞书 / 钉钉 official_cli 或官方 webhook adapter，且默认关闭；个人微信、QQ、Telegram、Discord 和其他通道全部标记 future / unsupported，不进入本阶段实现范围。
