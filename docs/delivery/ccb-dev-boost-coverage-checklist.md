...
- Phase 11-14 已落地 session handoff、memory、agent、多模型与 Skills/Workflows/Hooks/本地 Plugin 主闭环；Phase 14 hardening 与 Phase 15-18 仍是后续设计覆盖 hook 实测、真实项目对账、双模型交叉审查、长期任务和桌面端预留。
...
| 15. 插件 / skills / workflow / hooks | CCB Skills/workflow；Hermes Skills；OpenCode 插件化；hooks 管道 | Phase 14 主闭环已实现，hardening 待处理 | Phase 14；Phase 16 skill 固化 | `LINGHUN_PHASED_DELIVERY_BLUEPRINT.md:1225-1348`；`LINGHUN_IMPLEMENTATION_SPEC.md:1614-1815`；`LINGHUN_FINAL_ARCHITECTURE_AND_ROADMAP.md:725-750`；`phase-14-skills-workflow.md` | 本地 skills/workflows/hooks doctor/plugin loader/启停/信任/权限边界已落地；hook timeout/logPath 实测、输出截断实测、schema 兼容性和 workflow 结束检查仍属 Phase 14 hardening；GitHub 安装、插件市场、自动更新、长期任务仍未实现 | 保持 Phase 14 主闭环口径；hardening 只补稳定性和诊断实测，不把插件市场、GitHub 安装、自动更新或长期任务误写成已完成 |
...
- Phase 14：主闭环已实现 Skills、Workflows、Hooks doctor、本地 Plugin loader / doctor / 启停 / 信任 / 权限接入；Phase 14 hardening 待处理 hook timeout/logPath 实测、输出截断实测、schema 兼容性和 workflow 结束检查；插件市场、GitHub 安装、自动更新、长期任务和 Phase 15+ 能力仍未实现。
