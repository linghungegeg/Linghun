...
## 7.13.1 hotfix：复合输入 command-first 与稳定点识别

- 修复 `UserStateDecision` 单标签固定优先级导致的复合输入误路由：`frustrated` / `trust_repair` / `high_stakes_release` 保留主 `kind`，但可叠加 `decisive_command` 的 `interactionPlan.commandFirst=true` 与 `detailPlan.command_first/minimal`。
- `high_stakes_release` 仍是最高优先级；新增稳定点识别：`稳定点`、`提交稳定点`、`建立稳定点`、`stable point`、`checkpoint`、`commit point`。
- `high_stakes_release + decisive_command` 保持 release verification：`dirty-tree`、`untracked-files`、`build`、`focused-test`、`stability-boundary` 路由不降级。
- `frustrated` / `trust_repair + decisive_command` 保持 source facts、strengthened verification、forbid early PASS，只把交互与细节层改为命令优先。
- `strategic_exploration` / `confused` 不叠加 command-first；`先讨论架构取舍，不要实现代码。` 仍是 discussion-only，且不推动实现。
- 未修改权限管道、final answer gate、verification gate、provider breaker、workflow/job/agent gate；未触碰 capability-runtime、`WHITEPAPER*.md`、`docs/stress`、`img`、`report.md`、`test-model-set.sh`、DH1-DH4。
