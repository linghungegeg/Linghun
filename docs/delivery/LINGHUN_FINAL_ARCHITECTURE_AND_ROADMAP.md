...
| Phase 15.5 | 双模型交叉审查、MCP / Skills / Plugins Connect Lite、Bundled codebase-memory Lite、Compact Lite、Local Resource Guard Lite、Verification / Review Runtime Lite、Write/Edit/MultiEdit CCB-grade editing UX、模型接入成熟度、联网取证成熟度、终端 TUI polish 清零与开源前 hardening | GPT-5.5/Claude 做产品架构审查，DeepSeek V4 Pro 做代码安全审查，并补 Solution Completeness Gate 复检、provider adapter/capability doctor/usage-cache/quota/error/fallback/config、Freshness Gate/web_source evidence、随包 codebase-memory 固定版本/doctor/license 降级、Compact Lite 自动 MicroCompact/手动 compact/受控 autoCompact、Local Resource Guard Lite 的前台并发 1、后台任务 cap、分类型 cap、重任务互斥、输出落盘、timeout/cancel/stale 和 Windows process cleanup、Verification / Review Runtime Lite 的 cancel/timeout/stale/log/evidence/review verdict scope、Write/Edit/MultiEdit read-before-edit/diff preview/stale file、参考 Warp/OpenCode/CCB 的轻量 block/panel 终端产品细节、release readiness / open-source readiness；不得把 Phase 15 Beta 已需的基础 TUI 手感留到本阶段；终端开源前 terminal-scope P0/P1/P2 必须清零、降级为 NOT-DO，或证明不属于终端发布范围 |
...
- 2026-05-21 Phase 15 Pre-Smoke Full Reference Parity Audit + Permission / Architecture Runtime Feasibility + Source-of-Truth Reset 仍是 pre-smoke 证据基线：`F:\Linghun\docs\audit\phase-15-pre-smoke-full-reference-parity-and-architecture-runtime-audit.md`。Batch A/B/C 已完成并通过 A-C 综合验收门，综合验收报告为 `F:\Linghun\docs\audit\phase-15-pre-smoke-a-c-combined-acceptance.md`。A-C 的 `READY_FOR_USER_DECISION_TO_START_REAL_PROJECT_SMOKE` 只作为历史 evidence，不再是当前直接 smoke 入口。用户已选择把真实环境实测后移到终端候选产品成熟之后，当前唯一下一步是 `F:\Linghun\docs\delivery\pre-open-source-terminal-product-completion-gate.md`：按阶段完成 Phase 15.5、Phase 16、Phase 17A/17B 的终端产品完整度门禁，再由用户决定是否进入真实全量实测。这不是 Beta PASS，也不是 smoke-ready PASS；门禁完成前不得启动真实全量实测、Phase 18 桌面端或开源发布。不得从 focused/mock/local/scoped PASS、Batch 3.5 PASS、单个 live text PASS、SKIPPED/PENDING smoke、silent-failure ban PASS、`READY_TO_IMPLEMENT`、Batch B 文档完成、A-C 综合验收 PASS 或单阶段 PASS 推断终端产品 ready。历史 P0 hardening / Deep Parity / Runtime Acceptance / Verdict Evidence closure 只作为证据输入，不作为 readiness proof。
