...
### Must Fix Before Real Test (9 项；S1/S2/S3 已由 Phase 7.5-A DONE；MC1/AW1/AW3/T1 已由 Phase 7.5-B DONE；PM1 已由 Phase 7.5-B.2 DONE)
...
| ID | 发现 | 代码路径 | 行号 | 风险 | 建议最小修复 |
|---|---|---|---|---|---|
| **MC1** | DONE in Phase 7.5-B：`estimateInputChars` depth>6 改用 bounded JSON.stringify（上限 2000 chars），添加深嵌套低估测试 | `compact-context.ts` | 174 | 已修复 | 见 `phase-07-5b-mainchain-capability-test-stability-closure.md` |

#### Agent / Workflow / Job (2 项)

| ID | 发现 | 代码路径 | 行号 | 风险 | 建议最小修复 |
|---|---|---|---|---|---|
| **AW1** | DONE in Phase 7.5-B：`RegistryWorkflowStep` 新增 path/content 字段，write 步骤走 `handleToolCommand("Write", ...)` → 既有权限管道 | `workflow-command-runtime.ts`, `agent-workflow-registry.ts` | — | 已修复 | 见 `phase-07-5b-mainchain-capability-test-stability-closure.md` |
| **AW3** | DONE in Phase 7.5-B：`stopSingleBackgroundTask` 在 `transitionDurableJob` 之前增加显式 `stopRunnerForDurableJob` 调用 | `background-control-runtime.ts` | — | 已修复 | 见 `phase-07-5b-mainchain-capability-test-stability-closure.md` |

#### 权限 (1 项)

| ID | 发现 | 代码路径 | 行号 | 风险 | 建议最小修复 |
|---|---|---|---|---|---|
| **PM1** | DONE in Phase 7.5-B.2：移除 `executeWorkflowStep`/`runWorkflowSteps` 中 3 处 `phaseStopPointConfirmed` 硬编码；新增 `phaseGateConfirmed` 持久化字段到 `WorkflowState.activeRun`；`executeRegistryWorkflowStep` 接入同一 gate；6 个行为测试（bridge unit + registry integration）覆盖 | `workflow-command-runtime.ts`, `tui-data-types.ts`, `workflow-agent-runtime-bridge.ts` | — | 已修复 | 见 `phase-07-5b-mainchain-capability-test-stability-closure.md` |

#### 测试 (1 项)

| ID | 发现 | 代码路径 | 行号 | 风险 | 建议最小修复 |
|---|---|---|---|---|---|
| **T1** | DONE in Phase 7.5-B：锚点断言替换为 3 个行为测试（contract satisfied/violated/decision-not-required），直接调用 `verifyFailureLearningContract` 验证返回值 | `index.test.ts` | — | 已修复 | 见 `phase-07-5b-mainchain-capability-test-stability-closure.md` |
...
Must Fix Before Real Test 的剩余 4 项问题（DH1-DH4）在 Phase 7.5-C 中修复并验证后，方可进入真实实测。
...
```json
{
  ...
  "criticalBlockersToRealTest": [
    ...
    "S1/S2/S3 resolved in Phase 7.5-A; MC1/AW1/AW3/T1 resolved in Phase 7.5-B; PM1 resolved in Phase 7.5-B.2.1; remaining blockers are DH1/DH2/DH3/DH4"
  ],
