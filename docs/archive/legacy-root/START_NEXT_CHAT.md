...
29. F:\Linghun\docs\audit\phase-15-pre-smoke-batch-b-architecture-runtime-source-of-truth.md
30. F:\Linghun\docs\audit\phase-15-pre-smoke-batch-c-architecture-runtime-minimal-implementation.md
31. F:\Linghun\docs\audit\phase-15-pre-smoke-a-c-combined-acceptance.md
32. F:\Linghun\docs\delivery\phase-15-5a-performance-context.md
...
- Phase 00-18 Design + Runtime Overdesign Full Audit 曾在 2026-05-19 合并为历史执行基线：`F:\Linghun\PHASE_15_BETA_CCB_MATURITY_REMEDIATION_BASELINE.md`。该历史基线只作为 evidence / traceability，不再作为当前执行入口；当前唯一 pre-smoke 执行基线仍为 `F:\Linghun\docs\audit\phase-15-pre-smoke-full-reference-parity-and-architecture-runtime-audit.md`。2026-05-21 `docs/audit/phase-15-pre-beta-verification-guard-and-index-runtime-smoke.md` 只记录 Phase 15 Batch 1/2/3 focused/mock/local verification guard 与 MCP/index runtime smoke 的历史验证；Batch 3.5 以最新 Batch 3.5 报告和开发窗口验证结果作为历史 evidence；2026-05-20 `docs/audit/phase-15-ccb-grade-default-runtime-reconciliation.md` 只作为历史 reconciliation 输入和 pull-forward / keep-deferred 依据。Batch A/B/C 已完成并通过 A-C 综合验收门，综合验收报告为 `F:\Linghun\docs\audit\phase-15-pre-smoke-a-c-combined-acceptance.md`，当前 verdict 为 `READY_FOR_USER_DECISION_TO_START_REAL_PROJECT_SMOKE`。这不是 Beta PASS，也不是 smoke-ready PASS；未进入真实项目 smoke，未进入 Phase 15.5 / Phase 16+；用户确认前不得启动真实项目 smoke。
...
当前任务状态：Phase 15 Pre-Smoke Full Reference Parity Audit + Permission / Architecture Runtime Feasibility + Source-of-Truth Reset 已完成，pre-smoke 证据基线仍为 `F:\Linghun\docs\audit\phase-15-pre-smoke-full-reference-parity-and-architecture-runtime-audit.md`；Batch A/B/C 已完成并通过 A-C 综合验收门，综合验收报告为 `F:\Linghun\docs\audit\phase-15-pre-smoke-a-c-combined-acceptance.md`。A-C 的 `READY_FOR_USER_DECISION_TO_START_REAL_PROJECT_SMOKE` 只作为历史证据，不再是当前直接 smoke 入口。用户已选择停止“边实测边补底座债”的节奏，当前入口是 `F:\Linghun\docs\delivery\pre-open-source-terminal-product-completion-gate.md`。截至 2026-05-24，Phase 15.5A-F、Phase 16、Phase 17A/B/C、Pre-Smoke TUI Polish A-D、Performance Gate 小范围修复、Closure A、Closure B 和 Closure C 已完成对应 local/focused/mock/synthetic validation 或本地口径收口；这些只代表 scoped validation 或 local closure 闭合，不代表 Beta PASS、smoke-ready 或 open-source-ready。下一步不是继续补 P1/P2，而是由用户确认是否进入用户指定真实项目的 Real Provider + Real Project Smoke；如 smoke 发现真实问题，再按问题修复。真实 smoke 可通过 shell env 或本机私有 provider.env 使用 provider key；不得把真实 key 写入文档、报告、日志或项目 `.linghun/settings.json`，不得保存 raw provider request、完整 provider response 或完整日志。旧的 focused/mock/local/synthetic/scoped PASS、Batch 3.5 PASS、单个 live text PASS、SKIPPED/PENDING smoke、silent-failure ban PASS、`READY_TO_IMPLEMENT`、Batch B 文档完成、A-C 综合验收 PASS、单阶段 PASS、Performance Gate synthetic PASS、Closure A local PASS、Closure B local closure 都不得推断终端产品 ready。
...
- Phase 15 pre-smoke 新基线已写入 `F:\Linghun\docs\audit\phase-15-pre-smoke-full-reference-parity-and-architecture-runtime-audit.md`；Batch A/B/C 已完成并通过 A-C 综合验收门，综合验收报告为 `F:\Linghun\docs\audit\phase-15-pre-smoke-a-c-combined-acceptance.md`。A-C verdict 只作为历史 evidence；当前已升级为开源前终端产品完整度门禁 `F:\Linghun\docs\delivery\pre-open-source-terminal-product-completion-gate.md`。该门禁完成并综合验收前，不进入真实全量实测、Phase 18 桌面端或开源发布。
...
- Architecture Runtime 是 Phase 15 smoke 前必须拉入的底层工程判断能力，负责工程方向、技术选择、拆解、风险、验证路线和长期可维护性；它不是第五个权限模式、不是 Plan Mode、不是 skill、不是 prompt-only 文案，必须与 Anti-Hallucination Runtime 的 facts/evidence/source/boundaries 协作。v1 必须保持轻量：普通对话链路前的工程判断 guard，不新增 agent、ADR 数据库、完整 spec 平台或强制计划模式；最小实现围绕 `shouldTriggerArchitectureRuntime`、`collectArchitectureFacts`、`formatArchitectureCard`、`detectArchitectureDrift`。短 Architecture Card 字段为 target、projectFacts、recommendedApproach、rejectedApproaches、stagedBreakdown、risks、verification、nonGoals；持久化只允许 transcript system_event、短 evidence_record 和 handoff latest 的 currentArchitectureCard 摘要，不写长期 memory/DB。涉及最新版本、provider/API 当前行为、社区项目现状、模型价格、安全公告、部署平台规则或第三方方案对比时，必须按需走 Freshness/Web Evidence，未授权联网或联网失败只能写 unknown/stale，不得把模型记忆写成当前事实；后续动作若新增未提及依赖/配置、扩散模块、跳过 verification、违反 nonGoals 或把 unknown/stale 当事实，必须提示 drift 并重新确认或更新 card。
- Phase 15 后新增 Phase 15.5：双模型交叉审查、MCP / Skills / Plugins Connect Lite、Bundled codebase-memory Lite、Compact Lite、Local Resource Guard Lite、Verification / Review Runtime Lite、Write/Edit/MultiEdit CCB-grade editing UX、provider maturity、Freshness/web evidence、release/open-source readiness 的终端运行部分、终端 TUI polish 清零与开源前 hardening。它现在属于开源前终端产品完整度门禁的一部分，必须按阶段完成后再真实全量实测；不得再把这些默认终端成熟项当作实测后补债。
...
- Phase 15 pre-Beta P0 hardening、Deep Parity、Runtime Acceptance、Verdict Evidence 和 CCB Maturity Remediation baseline 均作为证据输入；当前最新状态以“当前任务状态”段和最新 delivery/audit 报告为准。不得从历史 closure、focused/mock/local/synthetic/scoped PASS 或单个 live text PASS 推断 Phase 15 Beta readiness PASS。当前真实全量实测已后移；Phase 15.5A-F、Phase 16、Phase 17A/B/C、Polish A-D、Performance Gate 小范围修复、Closure A、Closure B 和 Closure C 已完成本地/聚焦/模拟/合成验证或本地口径收口；下一步必须由用户确认是否进入真实 provider + 真实项目 smoke。
- 每个后续阶段完成后仍必须在 F:\Linghun\docs\delivery\ 下输出阶段交付文档；没有阶段交付文档，不视为阶段完成。
- 每次改动后说明验证结果和剩余风险。
- 自动工作默认只推进一个阶段；完成当前用户确认的阶段后必须停止，输出验证结果和 handoff packet。
- 当前不得直接进入真实全量实测、Phase 18 桌面端或开源发布；Pre-Smoke Closure C Provider/Auth Config Center 已完成后必须停止，输出交付报告、验证结果和剩余风险；是否进入用户指定真实项目的 Real Provider + Real Project Smoke 必须由用户确认。终端开源发布前 terminal-scope P0/P1/P2 必须清零、降级为 NOT-DO，或证明 out-of-scope。
