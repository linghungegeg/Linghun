...
| Phase 15 pre-smoke A-C combined acceptance | `F:\Linghun\docs\audit\phase-15-pre-smoke-a-c-combined-acceptance.md` | Batch A/B/C 已完成并通过 A-C 综合验收门的当前状态证据；当前 verdict 为 `READY_FOR_USER_DECISION_TO_START_REAL_PROJECT_SMOKE`，下一步只能由用户明确确认后进入 Phase 15 real-project smoke | 把 A-C 综合验收 PASS 写成 Beta PASS、smoke-ready PASS、真实项目 smoke 已开始/已通过，或写成已进入 Phase 15.5 / Phase 16+ | Phase 15 pre-smoke |
...
| Phase 15.5 | Phase 15 交付文档、双模型审查报告、release readiness、OpenSpec spec-driven workflow、OpenCode 输出边界与视觉层级、Warp block/panel 现代终端交互、CCB TUI/权限/提权/Plan/help/doctor/error/compact 交互边界、CCB task lifecycle / TaskOutput / TaskStop / terminal-status-before-cleanup 行为边界、CCB v2.4.3 discovery-before-execute 工具 guard、Bundled codebase-memory Lite、Verification / Review Runtime Lite、Write/Edit/MultiEdit editing UX、`packages/tui/src/index.ts` maintainability hardening、LiteLLM/OpenRouter/Vercel AI SDK 类 provider 抽象边界、WebSearch/WebFetch Freshness Gate 与 web_source evidence 边界 |
