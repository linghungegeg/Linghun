...
Pre-Smoke Ink TUI Product Shell Gate 的第一步结论：Linghun 当前终端主链路已经具备 provider/model、permission、Start Gate、tool output、doctor、background/job、readiness、NCB、config 等大量底层能力，但可见 TUI shell 仍是 `readline` + `writeLine` + 字符串 presenter 的 REPL 形态，不是成熟 Ink/React 产品壳。
...
- `packages/tui/package.json` 只依赖 `@linghun/config`、`@linghun/core`、`@linghun/providers`、`@linghun/shared`、`@linghun/tools`；没有 `ink`、`react` 或 `@anthropic/ink` 依赖。
- `apps/cli/src/cli.ts` 在无参数时动态 import `@linghun/tui` 并调用 `runTui()`；`--help` / `--version` 快速路径不会加载 TUI、模型、MCP、索引、验证器、插件或 cache 统计系统。
- `packages/tui/src/index.ts` 中 `runTui()` 是当前交互入口：加载 config/storage/session/tool/context/cache/mcp/index/memory/skills/workflows/hooks/plugins/remote/background 等运行时状态，随后通过 `writeLine()`、`writeStatus()`、`readInputLines()` 驱动 REPL。
- 当前 TUI 输入由 `node:readline/promises` 的 `createInterface` 和 `readInputLines()` 管理。TTY 路径设置 raw mode，并固定输出 `你> `；non-TTY 路径读取 stdin chunks 后按行 yield。
- 当前输出由 `writeLine(output, text)` 直接 `output.write(`${text}\n`)`；状态栏由 `writeStatus()` 拼接 `formatRuntimeStatusLine()`；首页由 `formatHomeScreen()` 拼接字符串。
- 当前首页文案是：中文 `项目 <project> · 模型 <model> · 模式 <mode>`，随后提示“可以直接说……需要精确命令时，用 /help 查看”；英文是 `Project <project> · Model <model> · Mode <mode>` 和 “You can describe a goal directly…”；不是成熟 composer / shell layout。
- 当前字典在 `messages` 中维护 `zh-CN` / `en-US`，但 `inputPrompt` 仍是 `你> ` / `you> `，且 `readInputLines()` 直接硬编码 `你> `，没有使用 `t(context, "inputPrompt")`。
- 当前 missing model / provider config 路径会输出“检测到还没有完成模型配置。输入 /model setup 填写 API 地址、API key、模型名称和推理等级。”并显示 provider.env 模板位置。该路径仍以 slash 命令为主路径。
...
- 没有 Ink/React renderer：当前 `@linghun/tui` 没有 Ink/React 依赖，也没有 `App.tsx`、component tree、snapshot renderer 或 view model rendering tests。
- 当前 shell 不是产品级 composer：输入区是 `你> ` readline prompt，没有成熟的 multi-line composer、placeholder、attachment/context pills、status tray、command palette、permission card 或 running card。
- 当前启动输出仍偏重：启动时依次输出 title、language/trust、status、home screen、intro、project rules hint、provider setup hint 等，容易把首次体验变成“报告/诊断流”。
- 内部术语仍有泄漏风险：源码中用户可见路径仍可能出现 `Start Gate`、`Plan approval`、`provider.env`、`endpointProfile`、`tools/tool_choice`、`tool_result`、`local/static only`、`readiness` 等；报告/doctor 可保留，主 shell 默认视图需降噪。
- 缺模型主路径仍依赖 slash：missing model 配置提示直接指向 `/model setup`，没有 first-run card、guided primary action、可取消/稍后配置路径和自然语言式默认 CTA。
- i18n 覆盖不完整：字典存在，但 `readInputLines()` 硬编码 `你> `；后续新增 shell 文案若直接写在 view 中会继续分散。
- Windows / PowerShell host 限制尚未产品化呈现：当前有 GB18030 decode、TTY 判断和 Windows CLI 兼容说明，但没有 mature shell 对 PowerShell/Windows Terminal/raw mode/ANSI/no-color/中文宽度限制的可见降级策略。
- 当前 view 与 controller 混杂：`runTui()` 同时负责状态初始化、输入循环、输出写入、signal handling、slash/natural/model loop，后续直接塞 Ink 会放大 `index.ts` 复杂度。
- 当前 tests 主要验证 focused text behavior；缺少 Ink snapshot、terminal width、unicode width、theme/no-color、non-TTY fallback 的产品壳回归。
...
- 窄终端 / no-color / non-TTY 覆盖：focused shell test 覆盖 80/60/40 columns、中文长路径、长模型名、no-color 文本 marker、dumb/plain/non-TTY fallback 条件；现有 `index.test.ts` 继续覆盖 piped stdin/headless runTui 行为。
- 单独复检命令结果：
  - `corepack pnpm exec vitest run packages/tui/src/index.test.ts`：PASS，176 tests passed。
  - `corepack pnpm exec vitest run packages/tui/src/shell`：PASS，6 tests passed。
