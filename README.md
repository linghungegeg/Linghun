...
- Phase 15 Pre-Smoke Full Reference Parity Audit + Permission / Architecture Runtime Feasibility + Source-of-Truth Reset 已完成，当前 pre-smoke 基线仍为 [docs/audit/phase-15-pre-smoke-full-reference-parity-and-architecture-runtime-audit.md](./docs/audit/phase-15-pre-smoke-full-reference-parity-and-architecture-runtime-audit.md)；Batch A/B/C 已完成并通过 A-C 综合验收门，综合验收报告为 [docs/audit/phase-15-pre-smoke-a-c-combined-acceptance.md](./docs/audit/phase-15-pre-smoke-a-c-combined-acceptance.md)。A-C 的 `READY_FOR_USER_DECISION_TO_START_REAL_PROJECT_SMOKE` 只作为历史证据；用户已选择把真实环境实测后移到终端候选产品成熟之后。当前唯一执行入口为 [docs/delivery/pre-open-source-terminal-product-completion-gate.md](./docs/delivery/pre-open-source-terminal-product-completion-gate.md)：按阶段完成 Phase 15.5、Phase 16、Phase 17A/17B 的终端产品完整度门禁，再由用户决定是否进入真实全量实测；Phase 18 桌面端和开源发布物料只做预留。
