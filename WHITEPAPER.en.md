...
Linghun is no longer just "a terminal UI around a model." It decomposes AI coding into composable modules: model understanding, tool execution, permission decisions, evidence recording, verification loops, cache cost, long-term context, failure reflection, Git stable points, multi-agent work, local intelligent orchestration, and local process supervision. These modules are then connected to one auditable engineering mainline.
...
Together, these three ideas form Linghun's long-term design principle: use a strong foundation to carry strong models, use engineering discipline to reduce uncertainty, and use low learning cost to let ordinary developers benefit in real work. Users see a developer workbench that can chat, execute, verify, roll back, decompose complex tasks, and accumulate experience. Internally, the system is a runtime that continuously constrains facts, permissions, cost, orchestration, and risk.

Controlled memory, self-learning, and reflection make this workbench avoid starting from zero every turn. Linghun gradually adapts to the user's wording, verification habits, project rules, and common commands, reducing the cost of re-explaining the same context. In later similar tasks, the model can work more smoothly, drift less often, and repeat fewer mistakes. Cross-turn personality continuity takes this further: the system remembers not only "what the user likes" but "how many consecutive failures have occurred, whether trust is rising or falling, whether this is turn 3 or turn 30, and whether the task domain just switched." This prevents the orchestration judgment from re-evaluating every turn from scratch.
...
- It uses local intelligent orchestration to decompose complex goals into Workflow Plans, phases, slices, roles, risk hints, runtime proposals, and evidence requirements, then reuses the architecture, job, fork, agents, verification, Git, memory, failure learning, and remote-summary mainline to move work forward.
...
| Central orchestration / Policy Kernel | Condenses task type, risk, permissions, evidence, memory, failure learning, user state, provider state, workflow/agent state, context pressure, architecture boundaries, Windows/terminal capability, and verification needs into a structured policy. Six gates (compact-before-provider, blocked-runtime stop, retry guard, final answer gate, verifier preference, failure-learning capture) are structurally enforced by the mainline subsystems rather than only injected into the prompt, closing the decision-to-execution loop. The model reasons and generates; the runtime chooses the route, emits light hints, sets verification requirements, reduces cost, and controls risk. |
| Intent classification | Upgraded from single-label regex matching to a signal-aware multi-intent classifier: continuity signals (failure/success/domain switch/trust score) take priority → weighted keyword scoring → fallback to model clarification when intent is unclear. Outputs primary and secondary intents so the scheduler can simultaneously prepare "read files" and "pre-warm write confirmation" in a single round. No hard guesses, no misclassification. |
| User State Routing and Personality Continuity | Per-turn perception: turns confusion, frustration, trust repair, strategic exploration, urgency, or fatigue into orchestration signals. It affects output density, source-first priority, verification strength, eagerness to open background work, and final answer boundaries. Cross-turn continuity: tracks consecutive failure/success counts, trust score, task domain switches, state persistence, and total turns, so the orchestration judgment carries "the trajectory of the last ten turns" rather than re-evaluating from scratch each time. Neither layer bypasses permission, evidence, or verification systems. |
...
| Central orchestration | When each subsystem acts alone, the model still has to decide on the fly whether to read files, edit, verify, switch provider, open a workflow, or call an agent, which can raise cost, drift routes, or over-open background work. | Policy Kernel condenses memory, failure learning, evidence, permissions, architecture, provider, context, workflow/agent, and platform state into structured policy. Six gates (compact, blocked runtime, retry, final answer, verifier, failure learning) are structurally enforced by mainline subsystems, not only injected as model reference text. | Users do not need to manually orchestrate complex routes; after choosing a route, the system uses code paths to ensure it is followed, rather than merely "suggesting" it to the model. |
...
| Verification and readiness | Focused tests, mocks, and synthetic smoke are easily described as "all passed", with gaps only discovered before release. | Verification level, readiness, review, problems, final answer gate. The verification runner selects domain-specific commands (code_change / documentation / tui_interactive, etc.) based on the scheduler's verificationRoute, avoiding a one-size-fits-all full run. | Users can see what was verified and what was not, reducing pre-release rework. |
...
Before each model turn, the central orchestration system evaluates whether the current round is expected to involve mutating actions (expectedMutating) and whether an explicit permission gate is required (requireExplicitGate). The permission engine reads these signals before tool execution to pre-warm the confirmation level — the user sees a more accurate prompt before a write action, rather than a pop-up after the fact. This does not change the four permission modes themselves; it only gives the permission judgment an extra layer of context.
...
The verification runner no longer runs the same full suite for every task. The central orchestrator recommends a domain-specific command set based on the task domain (code_change / documentation / tui_interactive / provider_model_config / agent_job_workflow / general). The verification runner selects by domain — typecheck+test+lint for code changes, markdown+link+frontmatter for documentation — reducing irrelevant verification overhead.
...
- **Cross-turn personality continuity**: built on top of controlled memory and failure learning, tracks consecutive failure/success counts, trust score, task domain switches, and total turns. The orchestration judgment carries "the trajectory of the last ten turns." This is not an emotional system; it enables the central layer to make more accurate task classification and risk judgment at cross-turn scale.
...
### 17.6 From Strategy Text to System Enforcement: Philosophy Module Closure

Policy Kernel's core proposition is replacing "dump all state into the prompt" with structured strategy. But strategy only completes the loop when the system actually enforces it. If the scheduler calculates "compact before provider this round," but compact still triggers unconditionally on its own; calculates "this round needs an explicit permission gate," but the permission engine has no idea; calculates "the verification route is code_change — run typecheck + test," but the verification runner never reads this route — then what the scheduler is really doing is still writing strategy notes for the model, not enforcing strategy routes for the system.

The philosophy module upgrade closes this loop. The six boolean gates produced by the scheduler — final answer gate, verifier preference, retry guard, compact-before-provider, blocked-runtime stop, failure-learning capture — are upgraded from "injected into the system prompt for the model's reference" to "system-level constraints that the mainline must check before calling the model, executing tools, triggering verification, compacting context, or launching agents."

This does not mean the scheduler replaces the mainline. The permission engine, verification runner, provider router, architecture guard, compact pipeline, and final answer gate still run independently with their own judgment logic. The scheduler's job is to tell them ahead of time "what type of task this round is, how high the risk, what operations are expected, and what route is recommended," so they gain an extra layer of context rather than flying blind.

Take the permission engine as an example: if the scheduler has already determined that the user's intent this round is "modify source code" with medium risk, the permission engine can pre-warm the confirmation level before tool execution — not bypassing user confirmation, but presenting a more accurate prompt at the right moment. Take the verification runner: if the scheduler has determined the task domain is documentation rather than code_change, the runner can prioritize markdown link and frontmatter checks rather than running a full typecheck + build + test suite.

The user-facing change after closure is not "one more panel." It is perceiving that the system does fewer wrong things at the wrong time — less hard-cramming when context is almost full, fewer immediate retries after a failure, fewer write-confirmation pop-ups when the user is clearly exploring options, and less packaging of local completion as overall readiness.

This is the final step for central orchestration to move from concept-complete to execution-complete. The scheduler's computation logic does not need to change — its 40-input, 14-dimension pure function is already production-grade and well-tested. What remains is to make every subsystem on the mainline look at what the scheduler just said before taking action.

### 17.7 Cross-turn Personality Continuity: Letting the System Remember "Where We Are"

The user state routing described in 17.3 is per-turn: each turn, the system evaluates the user's current input text independently to judge whether the user is confused, frustrated, commanding, or exploring. This is already a step forward from "reply the same way regardless of user state," but it has a fundamental limitation — it does not know what just happened.

Here is a real example: the user has been fixing the same bug for three turns, all failing. On the fourth turn, the user calmly types "try fixing that null pointer in auth.ts again." Judging purely from the text, the user appears neutral and clear — the system would classify this as neutral + decisive_command, proceed on the normal route, and not add extra verification pressure.

But how would an experienced engineering collaborator who actually understands the context behave? "You've failed on this three times already. This time I should be more careful — re-read the relevant code first, confirm I understand correctly before touching anything, run focused tests after changes, and definitely don't just say it's fixed."

This is what cross-turn personality continuity addresses. It is not about giving the system "emotions." It is about giving the system "memory" — remembering how many consecutive failures occurred, how many consecutive successes, what type of task the user was just doing, whether trust is rising or falling, and that this is not turn 1 of a fresh conversation.

Specifically, the continuity system tracks these cross-turn signals:

- **Consecutive failure count**: tool failures, provider failures, and verification failures accumulate. When the count reaches a threshold, the system automatically elevates source-first, explain-first, and verification-strengthened modes — without waiting for the user to say "be more careful."
- **Consecutive success count**: when multiple turns pass without failures and with passing verification, the system gradually restores normal verification levels and reduces unnecessary explicit gates. Trust is earned and can be progressively restored.
- **Task domain switch detection**: the user just switched from debugging auth (3 failures) to writing a README. When the system detects a domain switch, it automatically downweights failure learning from the previous domain, preventing debug experience from contaminating documentation work.
- **Trust score**: a 0–100 score accumulated from successes, failures, verification passes, and user corrections. When the score is low, the system is more conservative (source-first, verification required, no empty PASS claims). When the score recovers, the system returns to its normal rhythm.
- **User state persistence**: when the same user state persists across many turns, the system suppresses repeated hints to avoid saying "Strategy: source-first" on every single turn.
- **Total session turns**: long sessions (>30 turns) automatically enable compact preference to prevent context bloat.

The essence of this 200-line pure function is encoding "what an experienced engineer would do after consecutive failures" into system behavior. It does not replace UserStateSignal's per-turn perception — that still works. It adds a cross-turn trend layer on top. Per-turn perception answers "what state does the user's current sentence sound like?" The continuity system answers "given the trajectory of the last ten turns, should I be more cautious or more decisive?"

Like the rest of central orchestration, the continuity system has clear boundaries: it cannot auto-accept memory, cannot replace permission approval, cannot use "low trust score" as a reason to refuse user commands, and cannot make agents/jobs/workflows run by default. It only helps the central layer make more accurate task classification, risk judgment, and route selection at cross-turn scale.

This is also one of the fundamental design choices that differentiates Linghun from systems where "every turn is a new conversation." A long-term project is not 50 independent Q&A rounds — it is a continuous engineering flow. The system should know whether this is turn 3 or turn 30, whether it is the first attempt or the third retry, and whether the user is continuing the same task or has already switched to something new.

### 17.8 Scenario-based Benefit Estimate for Philosophy Module Closure + Personality Continuity

Taking the current capability as baseline (100), the estimated improvement from these two upgrades across different scenarios:

| Scenario | Improvement | Why |
| --- | --- | --- |
| Simple Q&A, basic coding | +5% ~ +15% | Simple scenarios do not need complex orchestration. Gains come mainly from policy consistency — less occasional forgetting to read files first, fewer interruptions by unnecessary gates. |
| Complex engineering tasks | +25% ~ +50% | In multi-stage work (locate → understand → modify → verify), the scheduler no longer merely "suggests" routes but enforces them — compact when needed, verify when needed, permission engine pre-warmed, no longer relying on the model's moment-by-moment judgment. |
| Long-term project maintenance | +50% ~ +100% | Cross-turn continuity lets the system know "this is turn 30, we just failed 3 times, trust score is low" — automatically elevating source-first and verification strength. Domain switches automatically isolate failure experience from the previous domain to prevent cross-domain contamination. Memory, failure learning, and continuity stack into three layers, making the system feel more like an experienced collaborator over time. |
| Multi-agent / multi-tool orchestration | +40% ~ +80% | The scheduler decides whether to suggest parallelism based on current agent count, workflow state, and resource pressure. The retry guard automatically degrades rather than hard-retrying when tools fail consecutively. Compact-before-provider avoids cramming context to the limit before fixing it. |
| Risk judgment and safety boundaries | +80% ~ +200% | All six gates upgraded from "model reference text" to system-level enforcement — the final answer gate no longer leaks empty PASS claims, blocked-runtime stop no longer lets the model be invoked while a workflow is stuck, the permission engine is pre-warmed before writes. This is the qualitative leap from "relying on model self-discipline" to "system guarantee." |
| Personality continuity and self-narrative | Qualitative change, hard to measure in percentages | This is not about giving the system "emotions" but about giving it "cross-turn memory" — automatically more cautious after consecutive failures, gradually restoring normal rhythm after consecutive successes, maintaining strategy stability across long sessions rather than re-judging from scratch each turn. Users perceive not a new panel but the system doing fewer wrong things at the wrong time. |

These numbers are architectural estimates, not precision measurements. The core logic remains: the scheduler's computation is already production-grade. The benefit comes not from "better calculation" but from "the calculated result being actually enforced."

### 17.9 Intent Classifier Upgrade: From Regex Matching to Signal-aware Understanding

Classifying user input into the correct `taskKind` is the first link in the scheduler's entire downstream chain — if the task kind is wrong, all subsequent routing, gates, and verification preferences drift. The current `classifyTaskKind` uses sequential regex matching, which essentially means "hit a keyword, pick a label." Three ceilings follow:

- **No memory**: does not know what the previous turn's task kind was, or whether the last turn failed.
- **No state**: whether the user is calm or frustrated, on a winning streak or a losing streak, the same keyword hits the same classification.
- **Single label**: when the user says "read auth.ts first, then see how to fix the null pointer," only one primary kind is captured; the secondary intent is discarded.

The upgraded classifier has three layers:

**Layer 1 — Continuity signals first**. Before regex, check the signals already computed by the cross-turn continuity system: consecutiveFailures ≥ 2 → prefer `code_fact` (read facts before acting); trustScore < 30 with no explicit user correction → prefer `code_fact`; task domain switched → prefer `chat` (align understanding in the new domain before editing); userStatePersistence ≥ 5 → dynamically adjust classification thresholds.

**Layer 2 — Weighted keyword scoring**. Retain regex coverage but without hard matching. When multiple keyword domains fire simultaneously, score by weight and output primary + one secondary instead of one label. For example, "read auth.ts and then see how to fix" hits both code_fact (weight 0.7) and edit (weight 0.5); output is primary=code_fact, secondary=edit — the scheduler uses both to simultaneously prepare file reads and pre-warm write confirmation.

**Layer 3 — Fallback to model clarification when intent is unclear**. When `intentUnclear = true`, the system does not guess locally. It defers to the model to ask the user for clarification. This prevents "can you check if that problem from before is still there" from being hard-matched into a code_fact grep.

The classifier remains a pure function — no model calls, no new dependencies. The change happens inside `classifyTaskKind` in `meta-scheduler-runtime.ts` and its immediate call site, roughly 100 lines.

Direct user impact:
- Mixed intents close in one round — no waiting for the system to finish reading before you add "and fix it too."
- After consecutive failures, the system automatically converges to "read source first, then act" — no need to manually switch strategy.
- Ambiguous expressions are not guessed at — one round of the system asking back saves more tokens than two rounds of manual correction.

This is the final piece of the puzzle for central orchestration: moving from "condensing state into policy" to "the first step of policy — understanding what the user actually wants — being itself state-aware."
...
- Phase: decomposes the goal into stages such as exploration, architecture confirmation, implementation, verification, cleanup, and handoff.
- Slice: splits each phase into smaller work units so the model does not compress a large goal into one vague answer.
- Role: assigns explorer, planner, worker, verifier, and similar responsibilities to slices, reducing the risk that all work is done from one model perspective.
- Risk hint: turns project rules, controlled memory, accepted learning preferences, and failure learning into planning risk hints.
- Runtime proposal: maps slices to existing job, fork, agents, verification, or details paths instead of creating a second execution system.
- Evidence requirement: states in advance which evidence can support PASS and which information is only context or status.
...
- **Architecture system**: architecture boundaries, impacted modules, frontend/TUI constraints, AntiCodeBlob, and code hygiene risks enter the plan by default, preventing complex tasks from drifting structurally from the start.
- **Permission system**: default, auto-review, plan, and full-access remain the only permission modes. Mutating slices only produce Start Gate and permission proposals; they do not bypass local permission checks.
- **Evidence and anti-hallucination**: critical slices carry evidence requirements. Agent summaries, job completion, remote events, and failure learning remain context/status only and cannot masquerade as PASS.
- **Verification system**: verification slices are part of the plan. Without verification evidence, the final delivery must remain PARTIAL or conservative.
- **Git stable points**: pre- and post-execution stable point/checkpoint suggestions help users keep rollback boundaries, but Linghun does not auto-commit.
- **Cache and budget**: the plan surfaces token, duration, cost, and multi-agent resource pressure. It does not turn an unset budget into a forced limit, nor invent cache hit rates.
- **Controlled memory and self-learning**: only project rules, memory summaries, and accepted preferences are read to make the plan better aligned with the project and user. They do not write memory automatically and are not current-fact evidence.
- **Reflection and failure learning**: historical provider, tool, verification, Git, index, and final-gate failures become risk hints so the model repeats fewer known mistakes.
- **Remote summary**: long tasks can project summary-first status to a phone, making it easier to monitor progress or approve pending actions away from the computer. The remote side is not a new executor.
...
- Current phase.
- Done/running/blocked/queued counts.
- Runnable, readonly, start-gate, and blocked request counts.
- Token, cost, and duration budget state.
- Architecture, Git, memory, failure-risk, and remote-summary hints.
- Next action.
- Mobile summary.
- Full request and evidence rows in details.

Evidence Merge separates "evidence that can support a completion claim" from "information that is only context or status." File reads, grep results, index queries, command output, test results, verification, provider observations, and architecture evidence can support PASS when they satisfy the rules. Agent summaries, job completion, remote events, failure learning, memory, and self-learning are context/status only; they cannot masquerade as verified completion.
...
- The user says, "Help me build a cache hit-rate report feature, split the work, and run verification." Linghun first generates a workflow plan.
- The system decomposes the task into exploration, architecture confirmation, implementation, verification, stable point suggestion, and handoff organization.
- The plan references project rules, memory summaries, accepted preferences, and historical failure risks so it does not start from zero every time.
- After user confirmation, exploration, implementation, verification, and cleanup can be assigned to the corresponding agent/job/verification paths.
- The user can continue the main session or leave the computer and monitor summary-first progress or approve pending actions through a remote channel.
- When the task ends, Linghun summarizes evidence, failures, verification scope, Git state, and next steps.
- The final answer claims completion only when evidence is sufficient; otherwise it downgrades conservatively.

The core advantage of this route is clear: strong models handle understanding and generation, while the strong foundation handles orchestration, permissions, evidence, verification, cost, memory, reflection, and recovery. Models can be replaced; the engineering runtime continues to provide the same controllable capabilities.
...
The minimal developer integration is intentionally thin. First, place a manifest inside the current project:
...
Second, the local application implements two HTTP endpoints:

```http
GET /linghun/capabilities
```

It returns either `{ "capabilities": [...] }` or a capability array. Linghun merges remote capability metadata with manifest metadata by id.

```http
POST /linghun/execute
```

The request body is generated by Linghun:
...
The application returns a bounded structured result:
...
The user-facing connection commands are:
...
DingTalk integration is split by the current code-level boundary:

```text
DingTalk custom robot webhook
-> notification-only outbound summaries
-> Linghun POSTs a msgtype/text payload
-> successful delivery is not mobile control and not verification PASS
```

The DingTalk webhook payload shape is:
...
If a signing secret reference is configured, Linghun appends `timestamp` and `sign` to the webhook URL using DingTalk robot signing. The secret is resolved only through an environment reference; it is not written to the main screen, transcript, or deliveryDetail. The user-facing checks are `/remote setup dingtalk`, `/remote test dingtalk`, `/remote status`, and `/remote events`. This webhook path can send redacted summaries, approval request summaries, job status, and verification result summaries; it cannot receive mobile messages.

To return DingTalk messages into Linghun, a DingTalk official app, Stream path, or equivalent bridge daemon is required. The current adapter recognizes DingTalk Stream frames with these key fields:
...
```json
{
  ...
  "data": {
    ...
    "text": {
      "content": "continue checking the failed test"
    }
  }
}
```

The frame is converted into a `RemoteInboundMessage`, then routed through the local remote inbox, active-turn guard, and permission pipeline. Approval text can only resume an existing local `pendingLocalApproval`; it cannot create a new write operation from a mobile message. For developers, DingTalk inbound integration means adapting DingTalk official app or Stream events into Linghun-recognized inbound messages while preserving messageId, source, expiry, signature/source proof, and bound user information.
...
- Decompose complex requirements into Workflow Plans, multi-agent subtasks, architecture confirmation, verification slices, stable point suggestions, risk hints, and final delivery evidence, reducing the cost of manual task breakdown and progress monitoring.
...
- Workflow Plan, Workflow Matrix, multi-agent work, and durable jobs are not automatic execution systems that bypass permission. They reuse the existing four permission modes, tool runtime, Evidence Merge, verification, architecture gate, and final answer gate. Background task completion does not mean delivery has been verified.
