...
export function formatAgentDetails(agent: AgentRun, context: TuiContext): string {
  ...
  const lines = [
    ...
    `- registryAgentId: ${agent.registryAgentId ?? "none"}`,
    `- maxTurns: ${agent.maxTurns ?? "default"}`,
    `- allowedTools: ${agent.allowedTools?.join(",") ?? "default"}`,
    `- status: ${agent.status}`,
    ...
    `- pendingMailbox: ${agent.mailbox.filter((message) => !message.consumedAt).length}`,
