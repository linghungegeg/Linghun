...
export function createModelSystemPrompt(
  ...
  const deferredReminder = options?.lightPath
    ? ""
    : formatDeferredToolsSystemReminder(context.language, snapshotDeferredTools(context));
  ...
  const commandCapabilityLine = options?.lightPath
    ? "CommandCapabilitySummary=ordinary_light_path_no_tools; answer directly. If files, commands, verification, index, agent, workflow, write, or current git/workspace facts are needed, say that tools are needed instead of inventing results."
