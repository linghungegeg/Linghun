...
export function formatRuntimeStatusLine(view: RuntimeStatusView, language: Language): string {
  ...
  const model = truncateDisplay(`${view.provider}/${view.model}`, 26);
  const session = truncateDisplay(view.session, 8);
  const line =
    language === "en-US"
      ? `Status: session=${session} model=${model} mode=${view.mode} bg=${view.background} cache ${cache} · index ${view.indexStatus} · gate ${view.gate}`
      : `[Linghun] 会话=${session} 模型=${model} 模式=${view.mode} 后台=${view.background} cache ${cache} · index ${view.indexStatus} · gate ${view.gate}`;
