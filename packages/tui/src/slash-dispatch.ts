...
function formatHelpAdvanced(language: Language): string {
  ...
  if (language === "en-US") {
    return [
      ...
      "  /fork [session|chat]   Fork this chat; /fork <type> <task> starts an agent",
      ...
      "  /compact              Run compact for long conversation context",
      "  /compact status       Show compact/context status without running compact",
      "  /context              Show context status without provider requests",
  ...
  return [
    ...
    "  /fork [session|chat]   复制当前会话；/fork <类型> <任务> 启动 agent",
    ...
    "  /compact              执行长对话上下文压缩",
    "  /compact status       只读查看 compact/context 状态",
