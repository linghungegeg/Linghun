...
describe("mcp-index-runtime", () => {
  ...
  test("resolveCodebaseMemoryBinary ignores legacy npx codebase-memory config and keeps bundled first", async () => {
    const projectPath = await mkdtemp(join(tmpdir(), "linghun-codebase-memory-legacy-"));
    const platformArch = "win32-x64";
    const bundledRoot = join(process.cwd(), "apps", "cli", "bundled");
    const binary = join(
      bundledRoot,
      "codebase-memory",
      platformArch,
    ...
    const previousCliBundledRoot = process.env.LINGHUN_CLI_BUNDLED_ROOT;
    process.env.LINGHUN_CODEBASE_MEMORY_PLATFORM_ARCH_TEST = platformArch;
    process.env.LINGHUN_CODEBASE_MEMORY_BUNDLED_DIR = bundledRoot;
    process.env.LINGHUN_CLI_BUNDLED_ROOT = bundledRoot;
    configureMcpIndexRuntime({
      getCurrentFreshness: () => ({} as never),
      writeStatus: () => undefined,
      checkBackgroundStartGuard: () => null,
      ...
      appendBackgroundTaskEvent: async () => undefined,
      rememberEvidence: () => undefined,
      resolveCodebaseMemoryPackageRoot: () => undefined,
    });
    const context = {
      ...createIndexContext(projectPath),
      config: {
    ...
    try {
      const resolution = await resolveCodebaseMemoryBinary(context as never);
