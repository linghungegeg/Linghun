...
本地智能调度是这套哲学继续向前的一层。Linghun 的方向是把复杂目标先拆成工作流计划，再把计划映射到现有架构系统、权限系统、job、fork、agent、verification、Git 稳定点建议、缓存预算、记忆摘要、失败风险和 evidence merge 主链。模型负责理解和推理，Linghun 负责把推理变成受控的工程单元：谁负责探索、谁负责执行、谁负责验证、哪里需要权限、哪些历史失败需要避开、哪些证据可以支持 PASS、哪些只能作为上下文。

这不是单独增加一套复杂模式，而是把已有四档权限、多智能体、长任务、证据、架构、记忆、反思和验证系统组合成更低学习成本的产品体验。用户只需要说清目标，系统负责判断它是一轮普通对话、一个后台任务、一个多智能体拆分，还是一个需要 Workflow Matrix 的复杂工程流程。
...
| 本地智能调度 / Workflow Matrix | 自然语言复杂目标进入 Workflow Plan；目标被拆成 phase、slice、role、risk hint、runtime proposal 和 evidence requirement；执行层复用 `/job`、`/fork`、`/agents`、verification、details、架构检查、Git 稳定点建议、记忆摘要、失败风险和远程摘要；Task Surface 汇总进度，Evidence Merge 判断证据强度。 |
...
## 17. 本地智能调度、Workflow Matrix 与长任务托管

Linghun 支持本地智能调度、Workflow Matrix、多智能体和长任务托管。它的目标不是让模型“无限自动跑”，而是把复杂任务拆成有目标、有计划、有角色、有风险提示、有预算、有状态、有日志、有交接和有验证边界的托管流程。
...
Linghun 的本地智能调度把这些环节纳入同一条可观察链路。用户可以用自然语言描述复杂目标，系统先生成 Workflow Plan，把目标拆成 phase、slice、role、risk hint、runtime proposal 和 evidence requirement，再把可执行部分映射到现有架构系统、权限系统、`/job`、`/fork`、`/agents`、verification、details、Git 稳定点建议、记忆摘要、失败学习和远程摘要主链。用户不需要先学习一套“高级编排语言”，也不需要把模型输出复制成一堆命令；复杂任务会被组织成更像工程项目的结构。

### 17.1 Workflow Matrix：把复杂目标变成工程单元
...
### 17.5 Task Surface 与 Evidence Merge：用户看到的是状态，不是噪音
