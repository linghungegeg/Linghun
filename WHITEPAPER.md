...
| 中枢调度系统 / Policy Kernel | 把任务类型、风险、权限、证据、记忆、失败学习、用户状态、provider 状态、workflow/agent 状态、上下文压力、架构边界、Windows/终端能力和验证需求收敛成结构化策略；模型负责推理和生成，运行时负责选择路线、轻提示、验证要求、降本和风险控制。 |
| 用户状态调度 / User State Routing | 将用户的困惑、焦虑、信任修复、战略探索、急迫或疲劳等状态转成调度信号；影响输出密度、源码优先级、验证强度、后台任务开启倾向和最终回答边界，但不绕过权限、证据和验证系统。 |
...
| 中枢调度 | 各系统各管各的时，模型仍要临场决定读文件、编辑、验证、切 provider、开 workflow 或调用 agent，容易成本高、路线漂移或默认多开。 | Policy Kernel 把记忆、失败学习、证据、权限、架构、provider、上下文、workflow/agent 和平台状态收敛成结构化策略，并用轻提示投影给用户。 | 用户不用手动调度复杂路线；系统会优先选择更省、更稳、更符合当前仓库事实的下一步。 |
