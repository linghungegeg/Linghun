...
Phase 14 必须拆成“主闭环”和“hardening”两段交付。主闭环只完成本地 skills/workflows/hooks/plugin loader、doctor、启停、信任和权限接入；hardening 再补加载顺序稳定性、缓存 hash、失败隔离、hook 超时、大输出截断和工作流验收。GitHub 安装、插件生命周期和开发文档属于成品版 Plugin System 的后续能力，除非用户明确确认，不得混入主闭环导致阶段膨胀。
...
Phase 15 真实项目测试完成后，增加一次只读优先的双模型交叉审查，并把终端 TUI 的产品手感、输出层级、轻提示、权限/提权交互和开源前发布就绪一起收口，降低漏 bug、漏安全问题、漏产品体验问题和阶段边界偏移的概率。本阶段不新增 Phase 16+ 产品功能，不提前进入 Phase 16。
...
- `/help` 分组、自然语言问状态、自然语言问用法、自然语言发起安全动作、自然语言发起危险动作。
