...
- `default`：展示风险摘要后按规则执行，写入和 Bash 仍走权限管道。
- `acceptEdits`：只允许工作区内低风险编辑减少审批；Bash、联网、依赖、权限、第三方扩展、越界路径仍需审批或拒绝。
- `auto`：必须有可用的本地分类器/规则 gate；gate 不可用时降级为拒绝或 default，不得静默放行。
- `dontAsk`：只能在明确用户配置范围内生效，仍受硬拒绝、安全路径和高风险保护约束。
- `bypass`：必须本地显式 opt-in，不能由模型、自然语言桥、remote channel、workflow、agent 或 plugin 静默开启；开启前必须显示高风险说明和恢复方式。

Plan 交互必须参考 CCB 的公开行为边界但自研实现：进入 plan 要清晰说明“只规划不执行”；计划完成后至少要支持“批准计划但手动确认编辑”“批准计划并进入 acceptEdits 边界”“拒绝并继续讨论/反馈修改”三类决策。批准计划不等于授权所有后续工具，具体写入、Bash、联网、依赖和权限变更仍要走权限管道。
...
- 写配置、stateful refresh/init、权限规则、bypass、第三方 skill/plugin enable、记忆接受/删除、rewind restore、Bash、依赖安装、remote/job/hook 类高风险动作，不能只靠普通“确认”直通；必须要求用户输入 exact command、选择明确选项，或进入权限管道。
...
- `bypass` 和 `auto` gating 是 Beta 前硬边界：bypass 必须本地 opt-in；auto 必须检查本地 gate/classifier 可用，不可用时拒绝或降级。
...
- Phase 15 preflight 完成后，如果交互审查发现 Catalog 漂移、参数提取不足、Start Gate 风险确认不足、bypass/auto gating 不完整或测试矩阵不足，必须先做 Phase 15 preflight hardening；这仍属于 Phase 15 Beta 前置，不等于进入 Phase 15 Beta 或 Phase 15.5。
...
2026-05-21 状态口径：`docs/audit/phase-15-pre-beta-verification-guard-and-index-runtime-smoke.md` 已记录 Phase 15 Batch 1/2/3 focused/mock/local verification guard 与当前 MCP/index runtime smoke 通过本地/focused 验证；Batch 3.5 以最新 Batch 3.5 报告和开发窗口验证结果为准；2026-05-20 `docs/audit/phase-15-ccb-grade-default-runtime-reconciliation.md` 不再是最新执行状态，只作为历史 reconciliation 输入和 pull-forward / keep-deferred 回归门依据。Phase 15 Beta readiness 仍为 PARTIAL/BLOCKED，等待真实 provider + 真实项目 smoke；不得从 focused/mock/local/scoped PASS、Batch 3.5 PASS、单个 live text PASS、SKIPPED/PENDING smoke 或 silent-failure ban PASS 推断 Beta readiness PASS。当前唯一下一步只能是：完成 Batch 3.5 收尾后，先完成 Active Docs Source-of-Truth Hardening，再进入真实项目 smoke。后续阶段接手时除 baseline 第 12/13 节外，还必须复制该 reconciliation 中的 pull-forward / keep-deferred 决策：`PRE_BETA_P1` 只能在 Phase 15 Beta 前收口或明确阻塞，`BETA_WATCH` 必须进入真实测试观察表，`PHASE_15_5` / `NOT_DO` 不得因聊天记忆或旧审计口径回流为当前必做项。历史 audit、历史 PASS、READY、READY_TO_FIX、READY_FOR_USER_DECISION 和聊天记忆只能作为 evidence，必须被 START_NEXT_CHAT、docs/delivery/README 和本蓝图中的最新状态覆盖。若后续阶段发现某个后置项实际影响普通请求 -> model -> tool_use -> permission -> tool_result -> continuation -> final answer 默认链路，必须按 Phase 15 遗漏处理，而不是登记为普通后续 polish。Bundled codebase-memory Lite / Batch 3.5 仍按独立任务推进，当前不得假设 `docs/audit/phase-15-bundled-codebase-memory-lite.md` 已完成。
...
- Plan、acceptEdits、auto、bypass：每种模式的“能做什么、不能做什么、是否会写文件、是否仍需权限审批”必须在交互中可见，不能靠用户猜。
...
- Start Gate、权限审批、提权说明、Plan 模式、acceptEdits、auto、bypass 的核心交互。
