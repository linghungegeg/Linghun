...
- Native Local Job Runner / Process Supervisor 只能作为 Phase 17A 候选底层增强接入，且必须先完成 Native-vs-Node 同场景 benchmark 和平台 hardening：Windows MSVC/linker、签名、杀软误报、中文/空格路径；Unix/macOS process group/session cleanup；managed/bundled runtime 分发；`/doctor runner`；missing/crash/protocol mismatch fallback tests；以及 scheduler/evidence/resource guard integration。未完成前只能保留为 prototype/evaluation input，不能接入主链路或宣称 17A ready。
