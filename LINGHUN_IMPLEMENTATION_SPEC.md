...
- Native runner 正式主链路接入归属 Phase 17C Native Runner / Job Supervisor Gate。17A/17B 期间只能作为 prototype/evaluation input；Node/TUI 继续负责短任务和默认路径。17C 必须完成 Runner Resolver、Runner Adapter、Node fallback、`/doctor runner`、Windows 进程树清理、heartbeat/log/status supervisor、crash/missing/protocol mismatch fallback、scheduler/evidence/resource guard/log artifact/handoff recovery 集成，以及中文/空格路径和跨平台 process cleanup 验证。runner 只能执行 Linghun 已批准的 job spec，不能绕过 Start Gate / permission pipeline，不能成为第二套 scheduler / agent runtime；cancelled / timeout / stale / runner crash 不得生成 PASS evidence。
...
2026-05-21 pre-smoke 补充口径：`F:\Linghun\docs\audit\phase-15-pre-smoke-full-reference-parity-and-architecture-runtime-audit.md` 仍是 pre-smoke 证据基线；Batch A/B/C 已完成并通过 A-C 综合验收门，综合验收报告为 `F:\Linghun\docs\audit\phase-15-pre-smoke-a-c-combined-acceptance.md`。A-C 的 `READY_FOR_USER_DECISION_TO_START_REAL_PROJECT_SMOKE` 只作为历史证据，不再是当前直接 smoke 入口。用户已选择把真实环境实测后移到终端候选产品成熟之后，当前唯一下一步是 `F:\Linghun\docs\delivery\pre-open-source-terminal-product-completion-gate.md`：按阶段完成 Phase 15.5、Phase 16、Phase 17A/17B 的终端产品完整度门禁，再由用户决定是否进入真实全量实测。这不是 Beta PASS，也不是 smoke-ready PASS；门禁完成前不得启动真实全量实测、Phase 18 桌面端或开源发布。不得从 focused/mock/local/scoped PASS、Batch 3.5 PASS、单个 live text PASS、SKIPPED/PENDING smoke、silent-failure ban PASS、`READY_TO_IMPLEMENT`、Batch B 文档完成、A-C 综合验收 PASS 或单阶段 PASS 推断终端产品 ready。Phase 15.5/16/17/18 开工和任何 delta audit 仍必须读取最新 pre-smoke 报告、A-C 综合验收报告、终端产品完整度门禁文档、reconciliation 的 `Explicit pull-forward list`、`Explicit keep-deferred list` 和 baseline 第 12/13 节；已拉回的默认编码链路问题不得再登记为普通 polish。Phase 18 桌面端和开源发布物料只做预留；个人微信、插件/skill 市场、云同步、自动更新、商业化账号和完整远程工作台不得回流到当前门禁。
...
- Compact Lite 必须拆成默认自动 MicroCompact、手动 `/compact` 和可选 guarded auto compact。MicroCompact 只能本地清理模型上下文中的旧大工具结果，不调用 summarizer、不执行工具、不写文件，原始内容保留在 transcript/evidence/log/fullOutputPath，并记录 microcompact boundary、节省 token 估算和被清理 tool ids。完整 compact 只有手动 `/compact` 或用户 opt-in autoCompact 阈值触发时允许执行；开始前必须写 HandoffCheckpoint，结束后写 CompactBoundary，失败、超时或取消时必须保留原会话并给出恢复命令。
- Compact 裁剪窗口必须保护 provider API invariants：tool_use/tool_result 配对、assistant thinking/tool blocks 的同源消息边界、最近用户目标和必要 evidence refs 不得被切断。若调用模型摘要，必须走 summarizer role、token 上限、timeout、cancel 和失败降级；compact 过程中不得执行工具、改文件、写长期记忆或启动新开发任务。
- Verification / Review Runtime Lite 属于 Phase 15.5 开源前 hardening；若 Phase 15 真实项目 smoke 暴露复核卡死、取消无效、长日志污染主屏或阻塞普通请求链路，则按 Phase 15 遗漏回补。当前 `/verify` 的同步 runner、日志归档和 `test_result` evidence 是基础，不得宣称等于 CCB-grade verifier。实现必须补齐：`AbortSignal` 贯穿 verification 子进程、`/interrupt` 或明确 cancel 能终止正在跑的验证、timeout 后先 `SIGTERM` 再 grace kill、Windows 进程树/子进程不残留、stdout/stderr 流式落日志且主屏限流、background task heartbeat/stale 状态真实更新、`/background` / `/details output` 非阻塞查看、完成/失败/partial/cancelled/timeout 全部写入 transcript event 和 evidence、`/review` 只基于 diff/changedFiles/verification/evidence 给 scoped verdict。
...
- Pre-Open-Source Terminal Product Completion Gate 将终端候选产品所需能力统一放到真实全量实测前完成。Phase 15.5 必须拆成可验收小阶段：15.5A Performance & Context（Workspace Reference Cache / Virtual Workspace Cache、Compact、cache/status/index fast path）、15.5B Resource & Task Lifecycle、15.5C Editing & Tool UX、15.5D Connect Lite、15.5E Provider & Freshness、15.5F Terminal Product Readiness；再推进 Phase 16 可控学习、Phase 17A local durable jobs、Phase 17B 企业微信/飞书/钉钉 remote channels 第一版、Phase 17C Native Runner / Job Supervisor Gate。15.5A 的 Workspace Reference Cache / Virtual Workspace Cache 只能缓存有界、可失效、可重建的工作区引用摘要和状态，例如文件 stat、目录摘要、索引状态、RuntimeStatus、工具/能力摘要、evidence/log 路径引用和小型哈希；失效必须基于 mtime/size/hash、配置、tool schema、provider/model、index freshness、compact boundary、plugin/skill/hook 列表等稳定维度；不得缓存完整源码、完整聊天、完整日志、完整索引结果、密钥、token 或 provider raw request；缓存失败必须降级为原始读/索引路径，不得阻断主对话。Phase 18 桌面端完整实现、开源发布物料、插件/skill 市场、自动更新、云同步、商业化账号、个人微信和完整远程工作台继续后置。该门禁必须源码级实现和 focused tests，不能是 prompt-only / 文档补丁。
- Phase 15.5C 之后允许追加 Log Artifact Runtime Lite 和 Workspace Snapshot Lite 两个轻量底座增强。两者都是用户无感的降本增效层：Log Artifact Runtime 只为已知 log/output artifact 提供 bounded tail/grep/errors 切片，Workspace Snapshot Lite 只提供 bounded 文件状态 metadata 和 refs。它们必须复用现有 TUI/background/evidence/cache/index/context 路径，不新增第二套 provider/tool/permission/evidence/MCP/index/agent 系统；不得做 native/binary、数据库、LSP、全仓 watcher、后台常驻索引、自研代码图引擎、模型自动总结或 PASS 判定改写。若真实实现证明 TS/Node 不足，必须用基准证据另行评审，不能默认引入二进制。
- Fast Workspace Scanner 不属于当前必做 runtime。它只能作为 post-runner optional native helper：默认继续使用 TS/Node Workspace Snapshot Lite 和 codebase-memory；scanner 不做 code graph、semantic search、Git status 或 index stale verdict。启动 scanner prototype 前必须有 benchmark 证明 TS/Node snapshot 在大仓库或多 agent/job 共享 metadata 场景下成为瓶颈，并且 managed binary、doctor、fallback、packaging 路线已可控。
...
- 首次进入项目提示 fast index。
- `/index status` 默认只走 fast/local artifact 状态和 `index_status` 基本状态，不做慢 `detect_changes`。
- `/index status --fresh` 或 `/index check` 才允许调用 `codebase-memory-mcp cli detect_changes` 做 stale 检测；可用且发现变更时显示 `stale` / changedFiles / stale hint。
- `detect_changes` 不可用时必须清晰降级，不影响 `/index status` 基于 fast/local 状态和 `index_status` 展示基本状态。
